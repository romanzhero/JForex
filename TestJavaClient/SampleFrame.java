...
class SampleFrame extends JFrame implements EWrapper {
	...
	private IBTextPanel m_tickers = new IBTextPanel(
			"Market and Historical Data", false);
	...
	private JPanel createButtonPanel() {
		...
		JButton butCalculateImpliedVolatility = new JButton(
		...
		JButton butCancelCalculateImpliedVolatility = new JButton(
		...
		JButton butCancelCalculateOptionPrice = new JButton(
	...
	void onReqMktDepth() {
		...
		if (depthDialog == null) {
			depthDialog = new MktDepthDlg("Market Depth ID [" + dialogId + "]",
					this);
			m_mapRequestToMktDepthDlg.put(dialogId, depthDialog);

			// cleanup the map after depth dialog is closed so it does not
			// linger or leak memory
		...
		// req mkt data
		m_client.reqMktDepth(dialogId, m_orderDlg.m_contract,
	...
	void onCalculateImpliedVolatility() {
		...
		m_client.calculateImpliedVolatility(m_orderDlg.m_id,
				m_orderDlg.m_contract, m_orderDlg.m_order.m_lmtPrice,
				m_orderDlg.m_order.m_auxPrice);
	}
	...
	public void openOrder(int orderId, Contract contract, Order order,
		...
		// received open order
		String msg = EWrapperMsgGenerator.openOrder(orderId, contract, order,
				orderState);
		m_TWS.add(msg);
	}
	...
	public void updateMktDepth(int tickerId, int position, int operation,
		...
		if (depthDialog != null) {
			depthDialog.updateMktDepth(tickerId, position, "", operation, side,
					price, size);
		} else {
			System.err
					.println("cannot find dialog that corresponds to request id ["
							+ tickerId + "]");
		}
	...
	public void updateMktDepthL2(int tickerId, int position,
		...
		MktDepthDlg depthDialog = m_mapRequestToMktDepthDlg.get(tickerId);
		if (depthDialog != null) {
			depthDialog.updateMktDepth(tickerId, position, marketMaker,
	...
	public void deltaNeutralValidation(int reqId, UnderComp underComp) {
		String msg = EWrapperMsgGenerator.deltaNeutralValidation(reqId,
				underComp);
		m_TWS.add(msg);
	}
