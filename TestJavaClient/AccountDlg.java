...
public class AccountDlg extends JDialog {
	...
	void updatePortfolio(Contract contract, int position, double marketPrice,
...
class PortfolioTable extends AbstractTableModel {
	...
	void updatePortfolio(Contract contract, int position, double marketPrice,
			double marketValue, double averageCost, double unrealizedPNL,
			double realizedPNL, String accountName) {
		PortfolioTableRow newData = new PortfolioTableRow(contract, position,
		...
		Object getValue(int c) {
			...
			case 3:
				return m_contract.m_expiry == null ? null : ""
						+ m_contract.m_strike;
			case 4:
				return (m_contract.m_right != null && m_contract.m_right
						.equals("???")) ? null : m_contract.m_right;
			case 5:
...
class AcctValueModel extends AbstractTableModel {
	...
	void updateAccountValue(String key, String val, String currency,
			String accountName) {
		AccountTableRow newData = new AccountTableRow(key, val, currency,
				accountName);
		int size = m_allData.size();
		for (int i = 0; i < size; i++) {
