...
public class ComboLegDlg extends JDialog {
	...
	public void onAddLeg() {
		try {
			...
			int exemptCode = Integer
					.parseInt(m_exemptCode.getText().length() != 0 ? m_exemptCode
							.getText() : "-1");
			m_comboLegsModel.addComboLeg(new ComboLeg(conId, ratio, m_action
					.getText(), m_exchange.getText(), openClose, shortSaleSlot,
					m_designatedLocation.getText(), exemptCode));
		} catch (Exception e) {
			reportError("Error - ", e);
			return;
		}
