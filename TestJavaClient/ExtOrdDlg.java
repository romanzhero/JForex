...
public class ExtOrdDlg extends JDialog {
	...
	private JCheckBox m_optOutSmartRoutingCheckBox = new JCheckBox(
			"Opting out of SMART routing", false);
	...
	public ExtOrdDlg(OrderDlg owner) {
		...
		extOrderDetailsPanel
				.add(new JLabel("VOL: Hedge Delta Clearing Account"));
		extOrderDetailsPanel.add(m_VOLDeltaNeutralClearingAccount);
		extOrderDetailsPanel
				.add(new JLabel("VOL: Hedge Delta Clearing Intent"));
	...
	void onOk() {
		...
		try {
			...
			m_order.m_designatedLocation = m_designatedLocation.getText()
					.trim();
			m_order.m_exemptCode = Integer.parseInt(m_exemptCode.getText()
					.length() != 0 ? m_exemptCode.getText() : "-1");
			m_order.m_rule80A = m_rule80A.getText().trim();
			m_order.m_allOrNone = parseInt(m_allOrNone) != 0;
			m_order.m_minQty = parseMaxInt(m_minQty);
			m_order.m_overridePercentageConstraints = parseInt(m_overridePercentageConstraints) != 0;
			...
			m_order.m_optOutSmartRouting = m_optOutSmartRoutingCheckBox
					.isSelected();
			...
			m_order.m_deltaNeutralSettlingFirm = m_VOLDeltaNeutralSettlingFirm
					.getText().trim();
			m_order.m_deltaNeutralClearingAccount = m_VOLDeltaNeutralClearingAccount
					.getText().trim();
			m_order.m_deltaNeutralClearingIntent = m_VOLDeltaNeutralClearingIntent
					.getText().trim();
