...
public class SRLevelsFinder extends BasicTAStrategy implements IStrategy {
	...
	public class PriceZoneDesc {
		...
		protected List<TriggerDesc>
			oneBars = new ArrayList<TriggerDesc>(),
			twoBars = new ArrayList<TriggerDesc>(),
			threeBars = new ArrayList<TriggerDesc>();
		
		protected double 
			oneBarsTotalSignalSizes = 0,
			twoBarsTotalSignalSizes = 0,
			threeBarsTotalSignalSizes = 0;
		...
		public long totalSignals() { return noOfOneBars() + noOfTwoBars() + noOfThreeBars(); }
		public double totalSignalSizes() { return oneBarsTotalSignalSizes + twoBarsTotalSignalSizes + threeBarsTotalSignalSizes; }
		
		public int noOfOneBars() { return oneBars.size(); }
		public int noOfTwoBars() { return twoBars.size(); }
		public int noOfThreeBars() { return threeBars.size(); }
	...
	public void onBar(Instrument instrument, Period period, IBar askBar, IBar bidBar) throws JFException {
		if (!period.equals(Period.THIRTY_MINS) || !instrument.equals(Instrument.EURUSD))
			return;
		
		if (!tradingAllowed(bidBar.getTime()))
			return;
		
		TradeTrigger.TriggerDesc bullishTrigger = tradeTrigger.bullishReversalCandlePatternDesc(instrument, period, OfferSide.BID, bidBar.getTime());
		...
		TradeTrigger.TriggerDesc bearishTrigger = tradeTrigger.bearishReversalCandlePatternDesc(instrument, period, OfferSide.BID, bidBar.getTime());
	...
	public void onStop() throws JFException {
		String currReport = "Candle-based support/resistance levels for period " + conf.getProperty("testIntervalStart") + " - " + conf.getProperty("testIntervalEnd")
	...
	private String createSingleCandlesReport(List<TriggerDesc> candidates) {
		...
		for (TriggerDesc curr : candidates) {
			result += i++ + ". " + FXUtils.getFormatedTimeCET(curr.getLastBar().getTime()) + ": " + curr.type.toString()
			+ ", S/R level " + FXUtils.df4.format(curr.srLevel)
			+ ", channel position " + FXUtils.df1.format(curr.channelPosition)
			+ ", reversal size " + FXUtils.df1.format(curr.reversalSize * Math.pow(10.0, Instrument.EURUSD.getPipScale())) + " pips\n"; 
		}
		return result;
	}
	...
	private String createSRLevelsReport(List<TriggerDesc> candidates) {
		...
		double
			maxValue = candidates.get(0).srLevel,
			minValue = candidates.get(candidates.size() - 1).srLevel;
		
		// now create the price zones of 10 pips (experiments possible with ATR average etc)
		...
		for (double currBlockTop = maxValue; currBlockTop > minValue; currBlockTop -= 0.0010) {
			priceZones.add(new PriceZoneDesc(currBlockTop - 0.0010, currBlockTop));			
		}
		
		// now do the counting
		for (TriggerDesc currCandle : candidates) {
			for (PriceZoneDesc currPriceZone : priceZones) {
				if (currCandle.srLevel >= currPriceZone.bottom && currCandle.srLevel <= currPriceZone.top) {
					...
					// PriceZones mutually exclusive i.e. candle can belong only to one
					break;
				}
			}
		}
		
		// OK. Sort the zones in descending order of candle count and print the report
		...
		for (PriceZoneDesc currZone : priceZones) {
			result += "\n----------------------------------------------------------------------------------------------------------------\n" 
			+ i++ + ". " + FXUtils.df4.format(currZone.bottom) + " - " + FXUtils.df4.format(currZone.top) 
			+ ": total signals " + currZone.totalSignals() 
			+ " (1-bar signals " + currZone.noOfOneBars()
			+ ", 2-bar signals " + currZone.noOfTwoBars()
			+ ", 3-bar signals " + currZone.noOfThreeBars()
			+ ")\n----------------------------------------------------------------------------------------------------------------\n";
			if (currZone.noOfOneBars() > 0) {
				result += "One bar signals:\n";
				for (TriggerDesc curr : currZone.getOneBars()) {
					result += FXUtils.getFormatedTimeCET(curr.getLastBar().getTime()) + ": " + curr.type.toString() 
						+ ", reversal size " + FXUtils.df1.format(curr.reversalSize * Math.pow(10.0, Instrument.EURUSD.getPipScale())) + " pips\n"; 
				}
			}
		...
		// For experiment, sort the zones in descending order of reversal sizes and print such report
		Collections.sort(priceZones, new Comparator<PriceZoneDesc>() {
		    public int compare(PriceZoneDesc o1, PriceZoneDesc o2) {
		    	// in descending order of total signals found
		...
		for (PriceZoneDesc currZone : priceZones) {
			result += "\n----------------------------------------------------------------------------------------------------------------\n" 
			+ i++ + ". " + FXUtils.df4.format(currZone.bottom) + " - " + FXUtils.df4.format(currZone.top) 
			+ ": total signal sizes " + FXUtils.df1.format(currZone.totalSignalSizes() * Math.pow(10.0, Instrument.EURUSD.getPipScale())) 
			+ "\n----------------------------------------------------------------------------------------------------------------\n";
			if (currZone.noOfOneBars() > 0) {
				result += "One bar signals:\n";
				for (TriggerDesc curr : currZone.getOneBars()) {
