...
public class TwoTFStatsCollector extends AdvancedMailCreator implements
		IStrategy {
	...
	public void onBar(Instrument instrument, Period period, IBar askBar,
		...
		if (period.equals(basicTimeFrame)
				&& (lowestTF == null || !lowestTF.equals("30min")))
			return;
		...
		if (conf.getProperty("mailToDBOnly", "no").equals("yes")) {
			dbLogSignalStats(
					instrument,
					period,
					logLine.get(2).getFormattedValue(),
					period.equals(Period.THIRTY_MINS) ? createMailBody30min(
							instrument, period, bidBar, logLine)
							: createMailBody4h(instrument, period, bidBar,
									logLine));
	...
	private void sendReportMail(Instrument instrument, Period pPeriod,
		...
		if (pPeriod.equals(Period.FIVE_MINS)) {
			...
			mailSubject = new String("Take profit target hit for "
					+ instrument.toString() + " at bar starting "
			...
			mailSubject = new String("FX pair report for " + pPeriod.toString()
		...
		if (conf.getProperty("sendMail", "no").equals("yes")
				&& (!filterMail
						...
						|| (pPeriod.equals(Period.FIVE_MINS) && mailBody
								.length() > 0) || sendFilteredMail))
	...
	private boolean inPosition(Instrument instrument) throws JFException {
		...
		try {
			Scanner scanner = new Scanner(new FileInputStream(instrument
					.toString().replace("/", "") + "_open_orders.txt"));
			while (scanner.hasNextLine()) {
				StringTokenizer orderRecord = new StringTokenizer(
						scanner.nextLine(), ":");
				// format is <ticker>:<direction (LONG/SHORT)>:<PnL in
				// pips>:<open price>:<SL or 0 if now set>:<TP or 0 if not set>
				String ticker = null, direction = null, openPrice = null, PnLinPips = null, stopLoss = null, takeProfit = null;
				for (int tokenPosition = 0; tokenPosition < 6
						&& orderRecord.hasMoreTokens(); tokenPosition++) {
					String nextToken = orderRecord.nextToken();
					switch (tokenPosition) {
					case 0:
						ticker = new String(nextToken);
						break;
					case 1:
						direction = new String(nextToken);
						break;
					case 2:
						PnLinPips = new String(nextToken);
						break;
					case 3:
						openPrice = new String(nextToken);
						break;
					case 4:
						stopLoss = new String(nextToken);
						break;
					case 5:
						takeProfit = new String(nextToken);
						break;
					default:
						break;
					}
				}
				if (ticker.equals(instrument.toString())
						&& (direction.equals("LONG") || direction
								.equals("SHORT"))) {
					openDukaOrders++;
				}
			}
	...
	protected String createMailBody30min(Instrument instrument, Period pPeriod,
			IBar bidBar, List<FlexLogEntry> logLine) throws JFException {
		...
		// TODO: mail body became quite complex. Needs to be split into easier
		// to handle parts which are then composed into end result
		String mailBody = new String(), candlesText = new String(), noGoZonesText = new String(), lowRiskHighQualitySignals = new String();
		...
		IBar prevBar30min = history.getBar(instrument, pPeriod, OfferSide.BID,
				2);
		String lastTwo30MinBarsStats = new String(
				"Last bar candle stats: upper handle = "
						...
						+ "% / lower handle = "
						+ FXUtils.df1.format(tradeTrigger
								.barsLowerHandlePerc(prevBar30min)) + "%\n"), valueToShow = mailStringsMap
				.get("CandleTrigger30min"), bullishCandles = null, bearishCandles = null;
		...
		if (valueToShow != null && !valueToShow.toUpperCase().equals("NONE")) {
			...
			if (valueToShow.contains("BULLISH")) {
				...
				if (twoCandleSignals)
					...
					candlesText += "\n"
							...
							+ (candleDesc != null && candleDesc.length() > 0 ? " ("
									+ candleDesc + ")"
									: "");
				candlesText += " (bar StDev size: "
						+ mailStringsMap.get("barStat30min");
				if (valueToShow.contains("BULLISH_1_BAR"))
					candlesText += ")\n";
				else
					candlesText += ", combined signal StDev size: "
							+ mailStringsMap
				...
				if (barHighPerc > 100)
					candlesText += "WARNING ! Possible breakout, careful with going long ! Bar high channel position "
				...
				if ((hit = checkEntryZones(instrument,
					...
					lowRiskHighQualitySignals += "\nATTENTION: bullish candle trigger in long entry zone ! "
							+ hit.getHitText() + " !\n";
					sendFilteredMail = true;
				}
			}
			...
			if (valueToShow.contains("BEARISH")) {
				...
				candlesText += "pivot bar high channels position: BBands "
						...
						+ "%\n" + "(volatility30min (BBands squeeze): "
						+ mailStringsMap.get("volatility30min") + "%)";
				candlesText += "\nlast bar high 4h channel position: "
						+ mailStringsMap.get("barHighChannelPos4h") + "%\n";
				candlesText += lastTwo30MinBarsStats;
				double barLowPerc = (Double) mailValuesMap.get(
						"barLowChannelPos30min").getValue();
				if (barLowPerc < 0)
					candlesText += "WARNING ! Possible breakout, careful with going short ! Bar low channel position "
				...
				if ((hit = checkEntryZones(instrument,
					...
					lowRiskHighQualitySignals += "\nATTENTION: bearish candle trigger in short entry zone ! "
							+ hit.getHitText() + " !\n";
					sendFilteredMail = true;
				}
			}
			...
			if (mailStringsMap.get("StochState4h").contains("OVER")) {
				if (stochFast4h > stochSlow4h)
					candlesText += " and RAISING (" + stochsToShow
							+ " --> difference "
							+ FXUtils.df1.format(stochFast4h - stochSlow4h)
							+ ")";
				else
					candlesText += " and FALLING (" + stochsToShow
							+ " --> difference "
							+ FXUtils.df1.format(stochFast4h - stochSlow4h)
							+ ")";
			} else
				candlesText += " (" + stochsToShow + " --> difference "
						+ FXUtils.df1.format(stochFast4h - stochSlow4h) + ")";

			Double trendStrength30min = (Double) mailValuesMap.get(
					"MAsDistance30min").getValue();
			if (trendStrength30min.doubleValue() < -0.7)
				candlesText += "\n\n30min trend: FLAT (Strength: "
						+ mailStringsMap.get("MAsDistance30min")
						+ "); TrendID: " + mailStringsMap.get("TrendId30min");
			else
				candlesText += "\n\n30min trend: "
						+ mailStringsMap.get("TrendId30min") + " ("
						+ mailStringsMap.get("MAsDistance30min") + ")";

			Double stochFast30min = (Double) mailValuesMap
					.get("StochFast30min").getValue();
			Double stochSlow30min = (Double) mailValuesMap
					.get("StochSlow30min").getValue();
			stochsToShow = FXUtils.df1.format(stochFast30min) + "/"
					+ FXUtils.df1.format(stochSlow30min);
			candlesText += "\n30min momentum: MACDState "
					+ mailStringsMap.get("MACDState30min") + " / MACDHState "
					+ mailStringsMap.get("MACDHState30min") + " / StochState "
					+ mailStringsMap.get("StochState30min");
			if (mailStringsMap.get("StochState30min").contains("OVER")) {
				if (stochFast30min > stochSlow30min)
					candlesText += " and RAISING ("
							+ stochsToShow
							+ " --> difference "
							+ FXUtils.df1.format(stochFast30min
									- stochSlow30min) + ")";
				else
					candlesText += " and FALLING ("
							+ stochsToShow
							+ " --> difference "
							+ FXUtils.df1.format(stochFast30min
									- stochSlow30min) + ")";
				...
				candlesText += " (" + stochsToShow + " --> difference "
						+ FXUtils.df1.format(stochFast30min - stochSlow30min)
						+ ")";
		}
		valueToShow = mailStringsMap.get("CandleTrigger4h");
		if (valueToShow != null && !valueToShow.toUpperCase().equals("NONE")
				&& !valueToShow.toLowerCase().equals("n/a"))
			candlesText += (candlesText.length() > 0 ? ", 4h: " : "4h: ")
					+ valueToShow;
		...
		mailBody += get3TFOverview(instrument, pPeriod, bidBar, mailStringsMap,
				mailValuesMap);
		...
		if (!mailStringsMap.get("MACDCross30min").equals("NONE")
				|| !mailStringsMap.get("MACDCross4h").equals("NONE")) {
			mailBody += "\nMACD cross ! ";
			if (!mailStringsMap.get("MACDCross30min").equals("NONE"))
				mailBody += "MACD30min: "
						+ mailStringsMap.get("MACDCross30min") + " (StDevPos: "
						+ mailStringsMap.get("MACDStDevPos30min") + ")";
			if (!mailStringsMap.get("MACDCross4h").equals("NONE"))
				if (!mailStringsMap.get("MACDCross30min").equals("NONE"))
					mailBody += "; MACD4h: "
							+ mailStringsMap.get("MACDCross4h")
							+ " (StDevPos: "
		...
		mailBody += "\n\nChannel position:\n" + "last 30min bar high/low: "
				+ mailStringsMap.get("barHighChannelPos1d") + "%/"
				+ mailStringsMap.get("barLowChannelPos1d") + "%\n"
				+ "Channel width: " + mailStringsMap.get("bBandsWidth1d")
				+ " pips\n";
		// zameni sa racunanjem...
		/*
		 * + "last 30min bar high distance to channel top: " +
		 * mailStringsMap.get("bBandsTopBarHighDiff1d") + " pips\n" +
		 * "last 30min bar low distance to channel bottom: " +
		 * mailStringsMap.get("bBandsBottomBarLow1d") + " pips\n";
		 */
		...
		String candlesText1d = new String(), valueToShow1d = mailStringsMap
				.get("CandleTrigger1d"), bullishCandles1d = null, bearishCandles1d = null;
		boolean twoCandleSignals1d = false;

		if (valueToShow1d != null
				&& !valueToShow1d.toUpperCase().equals("NONE")) {
			...
			if (positionOfAnd > 0) {
				...
				String secondCandle = valueToShow1d
						.substring(positionOfAnd + 5);
				if (firstCandle.contains("BULLISH")) {
			...
			if (valueToShow1d.contains("BULLISH")) {
				...
				if (valueToShow1d.contains("BULLISH_1_BAR"))
					candlesText1d += " (bar StDev size: "
							+ mailStringsMap.get("barStat1d") + ")\n";
				else
					candlesText1d += ", combined signal StDev size: "
							+ mailStringsMap
									.get("bullishTriggerCombinedSizeStat1d")
							+ ", lower handle : "
							+ mailStringsMap
									.get("bullishTriggerCombinedLHPerc1d")
							+ "%"
							+ ", real body : "
							+ mailStringsMap
									.get("bullishTriggerCombinedBodyPerc1d")
							+ "%, ("
							+ mailStringsMap
									.get("bullishTriggerCombinedBodyDirection1d")
							+ "), upper handle : "
							+ mailStringsMap
									.get("bullishTriggerCombinedUHPerc1d")
							+ "%" + "\n";

				candlesText1d += "pivot bar low 1d BBands channel position: "
						+ mailStringsMap
								.get("bullishCandleTriggerChannelPos1d")
						+ "%"
						+ "\nKeltner channel position (pivot bar): "
						+ mailStringsMap
								.get("bullishCandleTriggerKChannelPos1d")
						+ "%\n";
				double barHighPerc = (Double) mailValuesMap.get(
						"barHighChannelPos1d").getValue();
				if (barHighPerc > 100)
					candlesText1d += "WARNING ! Possible breakout, careful with going long ! Bar high channel position "
							+ mailStringsMap.get("barHighChannelPos1d") + "%\n";
				...
				double avgHandleSize = tradeTrigger.avgHandleLength(instrument,
						...
						last1dBar, FXUtils.YEAR_WORTH_OF_1d_BARS);
				Double barHalf = new Double(last1dBar.getLow()
						+ (last1dBar.getHigh() - last1dBar.getLow()) / 2), bullishPivotLevel = (Double) mailValuesMap
						.get("bullishPivotLevel1d").getValue(), aggressiveSL = new Double(
						bullishPivotLevel.doubleValue() - avgHandleSize), riskStandardSTP = new Double(
						(last1dBar.getHigh() - bullishPivotLevel)
								* Math.pow(10, instrument.getPipScale())), riskAggressiveSTP = new Double(
						(last1dBar.getHigh() - aggressiveSL)
								* Math.pow(10, instrument.getPipScale())), riskStandardLMT = new Double(
						(barHalf - bullishPivotLevel)
								* Math.pow(10, instrument.getPipScale())), riskAggressiveLMT = new Double(
						(barHalf - aggressiveSL)
								* Math.pow(10, instrument.getPipScale()));
				candlesText1d += "\nBUY STP: "
						+ FXUtils.df5.format(last1dBar.getHigh()) + " (risks "
						+ FXUtils.df1.format(riskStandardSTP.doubleValue())
						+ "/"
						+ FXUtils.df1.format(riskAggressiveSTP.doubleValue())
						+ ")\nBUY LMT: "
			...
			if (valueToShow1d.contains("BEARISH")) {
				...
				if (valueToShow1d.contains("BEARISH_1_BAR"))
					...
					candlesText1d += ", combined signal StDev size: "
							+ mailStringsMap
									.get("bearishTriggerCombinedSizeStat1d")
							+ ", lower handle : "
							+ mailStringsMap
									.get("bearishTriggerCombinedLHPerc1d")
							+ "%"
							+ ", real body : "
							+ mailStringsMap
									.get("bearishTriggerCombinedBodyPerc1d")
							+ "%, ("
							+ mailStringsMap
									.get("bearishTriggerCombinedBodyDirection1d")
							+ "), upper handle : "
							+ mailStringsMap
									.get("bearishTriggerCombinedUHPerc1d")
							+ "%" + "\n";

				candlesText1d += "pivot bar high BBands channel position: "
						+ mailStringsMap
								.get("bearishCandleTriggerChannelPos1d")
						+ "%\n"
						+ "Keltner channel position (pivotBar): "
						+ mailStringsMap
								.get("bearishCandleTriggerKChannelPos1d")
						+ "%\n";
				double barLowPerc = (Double) mailValuesMap.get(
						"barLowChannelPos1d").getValue();
				if (barLowPerc < 0)
					candlesText1d += "WARNING ! Possible breakout, careful with going short ! Bar low channel position "
							+ mailStringsMap.get("barLowChannelPos1d") + "%\n";
				...
				double avgHandleSize = tradeTrigger.avgHandleLength(instrument,
						...
						last1dBar, FXUtils.YEAR_WORTH_OF_1d_BARS);
				Double barHalf = new Double(last1dBar.getLow()
						+ (last1dBar.getHigh() - last1dBar.getLow()) / 2), bearishPivotLevel = (Double) mailValuesMap
						.get("bearishPivotLevel1d").getValue(), aggressiveSL = new Double(
						bearishPivotLevel.doubleValue() + avgHandleSize), riskStandardSTP = new Double(
						(bearishPivotLevel - last1dBar.getLow())
								* Math.pow(10, instrument.getPipScale())), riskAggressiveSTP = new Double(
						(aggressiveSL - barLow)
								* Math.pow(10, instrument.getPipScale())), riskStandardLMT = new Double(
						(bearishPivotLevel - barHalf)
						...
						(aggressiveSL - barHalf)
								* Math.pow(10, instrument.getPipScale()));
				candlesText1d += "\nSELL STP: "
						+ FXUtils.df5.format(last1dBar.getLow())
						+ " (risks "
						+ FXUtils.df1.format(riskStandardSTP.doubleValue())
						+ "/"
						+ FXUtils.df1.format(riskAggressiveSTP.doubleValue())
						+ ")\nSELL LMT: "
						+ FXUtils.df5.format(last1dBar.getLow()
								+ (last1dBar.getHigh() - last1dBar.getLow())
								/ 2) + " (risks "
						+ FXUtils.df1.format(riskStandardLMT.doubleValue())
						+ "/"
						+ FXUtils.df1.format(riskAggressiveLMT.doubleValue())
		...
		Double channelTop4h = (Double) mailValuesMap.get("bBandsTop4h")
				.getValue(), channelBottom4h = (Double) mailValuesMap.get(
				"bBandsBottom4h").getValue();
		mailBody += "Last price distance from channel top: "
				+ FXUtils.df1.format((channelTop4h - bidBar.getClose())
						* Math.pow(10, instrument.getPipScale()))
				+ " pips, from channel bottom: "
				+ FXUtils.df1.format((bidBar.getClose() - channelBottom4h)
						* Math.pow(10, instrument.getPipScale())) + " pips\n";
		...
		if (trendStrength30min.doubleValue() < -0.7)
			mailBody += "FLAT (Strength: "
					+ mailStringsMap.get("MAsDistance30min") + "); TrendID: "
					+ mailStringsMap.get("TrendId30min") + ", ";
		else
			mailBody += "TrendId" + ": " + mailStringsMap.get("TrendId30min")
					+ " (" + mailStringsMap.get("MAsDistance30min") + ")\n\n";
		...
		mailBody += "bar range: "
				+ FXUtils.df1.format((barHigh.doubleValue() - barLow
						.doubleValue())
						* Math.pow(10, instrument.getPipScale())) + " pips";
		mailBody += " (avg. bar range: "
				+ mailStringsMap.get("bar30minAvgSize") + ")\n";
		mailBody += "bar range vs. average range: "
				+ FXUtils.df1.format((Double) mailValuesMap.get("barStat30min")
						.getValue()) + " StDev(s) from average\n";
		double avgHandleSize = tradeTrigger.avgHandleLength(instrument,
				basicTimeFrame, OfferSide.BID, bidBar,
				FXUtils.MONTH_WORTH_OF_30min_BARS);
		mailBody += "avg. handle size: "
				+ FXUtils.df1.format(avgHandleSize
						* Math.pow(10, instrument.getPipScale())) + " pips\n";
		mailBody += "prevBarOverlap: "
				+ mailStringsMap.get("prevBarOverlap30min") + "%\n";

		mailBody += lastTwo30MinBarsStats;

		Double channelTop30min = (Double) mailValuesMap.get("bBandsTop30min")
				.getValue(), channelBottom30min = (Double) mailValuesMap.get(
				"bBandsBottom30min").getValue();
		mailBody += "\nLast price distance from channel top: "
				+ FXUtils.df1.format((channelTop30min - bidBar.getClose())
						* Math.pow(10, instrument.getPipScale()))
				+ " pips, from channel bottom: "
				+ FXUtils.df1.format((bidBar.getClose() - channelBottom30min)
						* Math.pow(10, instrument.getPipScale())) + " pips\n";
		mailBody += "Channel width: " + mailStringsMap.get("bBandsWidth30min")
				+ " pips\n\nMomentum:\n";

		mailBody += "MACDState: " + mailStringsMap.get("MACDState30min")
				+ " (StDevPos: " + mailStringsMap.get("MACDStDevPos30min")
				+ "), ";
		mailBody += "MACDHState: " + mailStringsMap.get("MACDHState30min")
				+ " (StDevPos: " + mailStringsMap.get("MACD_HStDevPos30min")
		...
		mailBody += "volatility30min (BBands squeeze): "
				+ mailStringsMap.get("volatility30min") + " %\n\n";
		...
		if (srLevels != null && srLevels.size() > 0) {
			...
			mailBody += showSortedSRLevels(instrument, new ArrayList<SRLevel>(
					srLevels), bidBar);
		}
	...
	private String get3TFOverview(Instrument instrument, Period pPeriod,
			IBar bidBar, Map<String, String> mailStringsMap,
			Map<String, FlexLogEntry> mailValuesMap) {
		String res = new String();
		final String colOneEmpties = new String("              |"), colOneLines = new String(
				"--------------|"), middleColHeader = new String(
				"                                                                           |"), middleColEmpties = new String(
				"                                      |"), middleColLines = new String(
				"--------------------------------------|"), horLineSeparator = colOneLines
				+ middleColLines + middleColLines + middleColLines + "\n", horEmptySeparator = colOneEmpties
				+ middleColEmpties + middleColEmpties + middleColEmpties + "\n";

		int replPos = (middleColHeader.length() - "Timeframe".length()) / 2, restPos = replPos
				+ "Timeframe".length();
		res += colOneEmpties
				+ FXUtils.replaceCentered(middleColHeader, "Timeframe") + "\n";
		// next line
		replPos = (colOneEmpties.length() - "Indicators".length()) / 2;
		restPos = replPos + "Indicators".length();
		res += colOneEmpties.substring(0, replPos) + "Indicators"
				+ colOneEmpties.substring(restPos) + middleColLines
				+ middleColLines + middleColLines + "\n";
		// next line
		...
		res += colOneLines + middleColLines + middleColLines + middleColLines
				+ "\n";
		// next line
		res += colOneEmpties + middleColEmpties + middleColEmpties
				+ middleColEmpties + "\n";
		// next line
		replPos = (colOneEmpties.length() - "Trend".length()) / 2;
		restPos = replPos + "Trend".length();
		res += colOneEmpties.substring(0, replPos) + "Trend"
				+ colOneEmpties.substring(restPos);

		boolean extraLine = false;
		if (mailValuesMap.get("MAsDistance1d").getDoubleValue() < -0.7) {
			res += FXUtils.replaceCentered(middleColEmpties, "FLAT ("
					+ mailStringsMap.get("MAsDistance1d") + ")");
			extraLine = true;
		} else {
			res += FXUtils.replaceCentered(
					middleColEmpties,
					mailStringsMap.get("TrendId1d") + " ("
							+ mailStringsMap.get("MAsDistance1d") + ")");
		}
		...
		if (extraLine) {
			res += colOneEmpties;
			if (mailValuesMap.get("MAsDistance1d").getDoubleValue() < -0.7) {
				res += FXUtils.replaceCentered(middleColEmpties, "("
						+ mailStringsMap.get("TrendId1d") + ")");
			} else {
		...
		res += FXUtils.replaceCentered(middleColEmpties,
				momentumCount(mailStringsMap, mailValuesMap, "1d"));
		res += FXUtils.replaceCentered(middleColEmpties,
				momentumCount(mailStringsMap, mailValuesMap, "4h"));
		res += FXUtils.replaceCentered(middleColEmpties,
				momentumCount(mailStringsMap, mailValuesMap, "30min"))
				+ "\n";
		// next line
		res += FXUtils.replaceCentered(colOneEmpties, "MACD");
		res += FXUtils.replaceCentered(middleColEmpties,
				mailStringsMap.get("MACDState1d"));
		res += FXUtils.replaceCentered(middleColEmpties,
				mailStringsMap.get("MACDState4h"));
		res += FXUtils.replaceCentered(middleColEmpties,
				mailStringsMap.get("MACDState30min"))
				+ "\n";
		if (mailStringsMap.get("MACDCross1d").equals("BULL_CROSS_BELOW_ZERO")
				|| mailStringsMap.get("MACDCross1d").equals(
						"BEAR_CROSS_ABOVE_ZERO")
				|| mailStringsMap.get("MACDCross4h").equals(
						"BULL_CROSS_BELOW_ZERO")
				|| mailStringsMap.get("MACDCross4h").equals(
						"BEAR_CROSS_ABOVE_ZERO")
				|| mailStringsMap.get("MACDCross30min").equals(
						"BULL_CROSS_BELOW_ZERO")
				|| mailStringsMap.get("MACDCross30min").equals(
						"BEAR_CROSS_ABOVE_ZERO")) {
			res += colOneEmpties;
			if (mailStringsMap.get("MACDCross1d").equals(
					"BULL_CROSS_BELOW_ZERO")
					|| mailStringsMap.get("MACDCross1d").equals(
							"BEAR_CROSS_ABOVE_ZERO"))
				res += FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("MACDCross1d"));
			else
				res += middleColEmpties;
			if (mailStringsMap.get("MACDCross4h").equals(
					"BULL_CROSS_BELOW_ZERO")
					|| mailStringsMap.get("MACDCross4h").equals(
							"BEAR_CROSS_ABOVE_ZERO"))
				res += FXUtils.replaceCentered(middleColEmpties,
				...
				res += middleColEmpties;
			if (mailStringsMap.get("MACDCross30min").equals(
					"BULL_CROSS_BELOW_ZERO")
					|| mailStringsMap.get("MACDCross30min").equals(
							"BEAR_CROSS_ABOVE_ZERO"))
				res += FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("MACDCross30min"))
						+ "\n";
			else
				res += middleColEmpties + "\n";
		}
		...
		res += FXUtils.replaceCentered(middleColEmpties,
				mailStringsMap.get("MACDHState1d"));
		res += FXUtils.replaceCentered(middleColEmpties,
		...
		res += FXUtils.replaceCentered(middleColEmpties,
				mailStringsMap.get("StochState1d"));
		res += FXUtils.replaceCentered(middleColEmpties,
				mailStringsMap.get("StochState4h"));
		res += FXUtils.replaceCentered(middleColEmpties,
				mailStringsMap.get("StochState30min"))
		...
		if (mailStringsMap.get("stochsCross1d").contains("CROSS_FROM"))
			res += FXUtils.replaceCentered(
					middleColEmpties,
					mailStringsMap.get("stochsCross1d")
							+ ", "
							+ FXUtils.df1.format(mailValuesMap.get(
									"StochFast1d").getDoubleValue()
									- mailValuesMap.get("StochSlow1d")
											.getDoubleValue()));
		else
			res += FXUtils.replaceCentered(middleColEmpties,
					FXUtils.df1
							.format(mailValuesMap.get("StochFast1d")
									.getDoubleValue()
									- mailValuesMap.get("StochSlow1d")
											.getDoubleValue()));
		if (mailStringsMap.get("stochsCross4h").contains("CROSS_FROM"))
			res += FXUtils.replaceCentered(
					middleColEmpties,
					mailStringsMap.get("stochsCross4h")
							+ ", "
							+ FXUtils.df1.format(mailValuesMap.get(
									"StochFast4h").getDoubleValue()
									- mailValuesMap.get("StochSlow4h")
											.getDoubleValue()));
		else
			res += FXUtils.replaceCentered(middleColEmpties,
					FXUtils.df1
							.format(mailValuesMap.get("StochFast4h")
									.getDoubleValue()
									- mailValuesMap.get("StochSlow4h")
											.getDoubleValue()));
		if (mailStringsMap.get("stochsCross30min").contains("CROSS_FROM"))
			res += FXUtils.replaceCentered(
					middleColEmpties,
					mailStringsMap.get("stochsCross30min")
							+ ", "
							+ FXUtils.df1.format(mailValuesMap.get(
									"StochFast30min").getDoubleValue()
									- mailValuesMap.get("StochSlow30min")
											.getDoubleValue()))
					+ "\n";
		else
			res += FXUtils.replaceCentered(middleColEmpties, FXUtils.df1
					.format(mailValuesMap.get("StochFast30min")
							.getDoubleValue()
							- mailValuesMap.get("StochSlow30min")
									.getDoubleValue()))
					+ "\n";
		...
		res += FXUtils.replaceCentered(middleColEmpties,
				channelPosDesc(mailStringsMap, mailValuesMap, "1d"));
		res += FXUtils.replaceCentered(middleColEmpties,
				channelPosDesc(mailStringsMap, mailValuesMap, "4h"));
		res += FXUtils.replaceCentered(middleColEmpties,
				channelPosDesc(mailStringsMap, mailValuesMap, "30min"))
				+ "\n";
		if (mailValuesMap.get("barsAboveChannel30min").getIntegerValue() > 0
				|| mailValuesMap.get("barsBelowChannel30min").getIntegerValue() > 0
				|| mailValuesMap.get("barsAboveChannel4h").getIntegerValue() > 0
				|| mailValuesMap.get("barsBelowChannel4h").getIntegerValue() > 0
				|| mailValuesMap.get("barsAboveChannel1d").getIntegerValue() > 0
				|| mailValuesMap.get("barsBelowChannel1d").getIntegerValue() > 0) {
			res += colOneEmpties;
			if (mailValuesMap.get("barsAboveChannel1d").getIntegerValue() > 0) {
				res += FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("barsAboveChannel1d")
								+ " bars ABOVE channel !");
			} else if (mailValuesMap.get("barsBelowChannel1d")
					.getIntegerValue() > 0) {
				res += FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("barsBelowChannel1d")
								+ " bars BELOW channel !");
			} else
				res += middleColEmpties;
			...
			if (mailValuesMap.get("barsAboveChannel4h").getIntegerValue() > 0) {
				res += FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("barsAboveChannel4h")
								+ " bars ABOVE channel !");
			} else if (mailValuesMap.get("barsBelowChannel4h")
					.getIntegerValue() > 0) {
				res += FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("barsBelowChannel4h")
								+ " bars BELOW channel !");
			} else
				res += middleColEmpties;
			...
			if (mailValuesMap.get("barsAboveChannel30min").getIntegerValue() > 0) {
				res += FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("barsAboveChannel30min")
								+ " bars ABOVE channel !");
			} else if (mailValuesMap.get("barsBelowChannel30min")
					.getIntegerValue() > 0) {
				res += FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("barsBelowChannel30min")
								+ " bars BELOW channel !");
			} else
				res += middleColEmpties;
		...
		// next section and line
		res += FXUtils.replaceCentered(colOneEmpties, "OS / OB")
				+ middleColEmpties + middleColEmpties + middleColEmpties + "\n";
		res += FXUtils.replaceCentered(colOneEmpties, "RSI");
		if (mailValuesMap.get("RSI1d").getDoubleValue() > 68.0)
			res += FXUtils.replaceCentered(middleColEmpties, "OVERBOUGHT ! "
					+ mailStringsMap.get("RSI1d"));
		else if (mailValuesMap.get("RSI1d").getDoubleValue() < 32.0)
			res += FXUtils.replaceCentered(middleColEmpties, "OVERSOLD ! "
					+ mailStringsMap.get("RSI1d"));
		else
			res += FXUtils.replaceCentered(middleColEmpties,
					mailStringsMap.get("RSI1d"));
		...
		if (mailValuesMap.get("RSI30min").getDoubleValue() > 68.0)
			...
			res += FXUtils.replaceCentered(middleColEmpties,
					...
					+ "\n";
		res += colOneEmpties
				+ FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("RSIState1d"))
				+ FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("RSIState4h"))
				+ FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("RSIState30min")) + "\n";
		...
		if (stochState.equals("OVERBOUGHT_BOTH"))
			res += FXUtils.replaceCentered(middleColEmpties,
					"OVERBOUGHT ! (" + mailStringsMap.get("StochFast1d") + "/"
							+ mailStringsMap.get("StochSlow1d") + ")");
		else if (stochState.equals("OVERBSOLD_BOTH"))
			res += FXUtils.replaceCentered(middleColEmpties,
					"OVERSOLD ! (" + mailStringsMap.get("StochFast1d") + "/"
							+ mailStringsMap.get("StochSlow1d") + ")");
		else
			res += FXUtils.replaceCentered(
					middleColEmpties,
					mailStringsMap.get("StochFast1d") + "/"
							+ mailStringsMap.get("StochSlow1d"));
		stochState = mailStringsMap.get("StochState4h");
		if (stochState.equals("OVERBOUGHT_BOTH"))
			res += FXUtils.replaceCentered(middleColEmpties,
					"OVERBOUGHT ! (" + mailStringsMap.get("StochFast4h") + "/"
							+ mailStringsMap.get("StochSlow4h") + ")");
		else if (stochState.equals("OVERBSOLD_BOTH"))
			res += FXUtils.replaceCentered(middleColEmpties,
					"OVERSOLD ! (" + mailStringsMap.get("StochFast4h") + "/"
							+ mailStringsMap.get("StochSlow4h") + ")");
		else
			res += FXUtils.replaceCentered(
					middleColEmpties,
					mailStringsMap.get("StochFast4h") + "/"
							+ mailStringsMap.get("StochSlow4h"));
		stochState = mailStringsMap.get("StochState30min");
		if (stochState.equals("OVERBOUGHT_BOTH"))
			res += FXUtils.replaceCentered(middleColEmpties, "OVERBOUGHT ! ("
					+ mailStringsMap.get("StochFast30min") + "/"
					+ mailStringsMap.get("StochSlow30min") + ")")
					+ "\n";
		else if (stochState.equals("OVERBSOLD_BOTH"))
			res += FXUtils.replaceCentered(middleColEmpties, "OVERSOLD ! ("
					+ mailStringsMap.get("StochFast30min") + "/"
		...
		res += colOneEmpties
				+ FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("CCIState1d"))
				+ FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("CCIState4h"))
				+ FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("CCIState30min")) + "\n";
		res += horEmptySeparator + horLineSeparator + horEmptySeparator;

		// next section and next line
		res += FXUtils.replaceCentered(colOneEmpties, "Candles");
		extraLine = false;
		boolean twoSignals1d = false, twoSignals4h = false, twoSignals30min = false, bullish1d = false, bearish1d = false, bullish4h = false, bearish4h = false, bullish30min = false, bearish30min = false;
		String candleTrigger = mailStringsMap.get("CandleTrigger1d"), candleText = null;
				...
				if (bullish4h) {
					res += FXUtils
							.replaceCentered(
									...
									"(1d: "
											+ mailStringsMap
													.get("bullishPivotLevelHigherTFChannelPos4h")
											+ "%/"
											+ mailStringsMap
					...
					res += FXUtils
							.replaceCentered(
									...
									"(1d: "
											+ mailStringsMap
													.get("bearishPivotLevelHigherTFChannelPos4h")
											+ "%/"
											+ mailStringsMap
		...
		if (twoSignals1d || twoSignals4h || twoSignals30min) {
			...
			if (twoSignals1d) {
				res += FXUtils.replaceCentered(
						middleColEmpties,
						mailStringsMap.get("CandleTrigger1d").substring(
								mailStringsMap.get("CandleTrigger1d").indexOf(
										" AND ") + 5));
			} else
				res += middleColEmpties;
			if (twoSignals4h) {
				res += FXUtils.replaceCentered(
						middleColEmpties,
						mailStringsMap.get("CandleTrigger4h").substring(
								mailStringsMap.get("CandleTrigger4h").indexOf(
										" AND ") + 5));
			} else
				res += middleColEmpties;
			if (twoSignals30min) {
				res += FXUtils.replaceCentered(
						middleColEmpties,
						mailStringsMap.get("CandleTrigger30min").substring(
								mailStringsMap.get("CandleTrigger30min")
										.indexOf(" AND ") + 5))
						+ "\n";
			} else
				res += middleColEmpties + "\n";
		...
		if (mailValuesMap.get("volatility1d").getDoubleValue() > 170.0)
			res += FXUtils.replaceCentered(middleColEmpties, "HIGH ! ("
					+ mailStringsMap.get("volatility1d") + "%)");
		else if (mailValuesMap.get("volatility1d").getDoubleValue() < 70.0)
			res += FXUtils.replaceCentered(middleColEmpties, "LOW ! ("
					+ mailStringsMap.get("volatility1d") + "%)");
		else
			res += FXUtils.replaceCentered(middleColEmpties,
					mailStringsMap.get("volatility1d") + "%");
		...
		res += FXUtils.replaceCentered(colOneEmpties, "ATR")
				+ FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("ATR1d") + " pips")
				+ FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("ATR4h") + " pips")
				+ FXUtils.replaceCentered(middleColEmpties,
						mailStringsMap.get("ATR30min") + ", +20% "
								+ mailStringsMap.get("ATR30min + 20%")
								+ " pips") + "\n";
	...
	private String channelPosDesc(Map<String, String> mailStringsMap,
			Map<String, FlexLogEntry> mailValuesMap, String timeFrameID) {
		String res = new String();

		if (mailValuesMap.get("barHighChannelPos" + timeFrameID)
				.getDoubleValue() > 90.0)
			res += "HIGH ("
					+ mailStringsMap.get("barHighChannelPos" + timeFrameID)
					+ "%/"
					+ mailStringsMap.get("barLowChannelPos" + timeFrameID)
					+ "%)";
		else if (mailValuesMap.get("barLowChannelPos" + timeFrameID)
				.getDoubleValue() < 10.0)
			res += "LOW ("
					+ mailStringsMap.get("barHighChannelPos" + timeFrameID)
					+ "%/"
			...
			res += mailStringsMap.get("barHighChannelPos" + timeFrameID) + "%/"
					+ mailStringsMap.get("barLowChannelPos" + timeFrameID)
					+ "%";
	...
	private String momentumCount(Map<String, String> mailStringsMap,
			Map<String, FlexLogEntry> mailValuesMap, String timeFrameID) {
		...
		double StochFast = mailValuesMap.get("StochFast" + timeFrameID)
				.getDoubleValue(), StochSlow = mailValuesMap.get(
				"StochSlow" + timeFrameID).getDoubleValue();
		if (StochFast > StochSlow)
			upCount++;
		else
			downCount++;

		String MACDState = mailStringsMap.get("MACDState" + timeFrameID), suffix = null;
	...
	protected String createMailBody5min(Instrument instrument, Period pPeriod,
			IBar bidBar) throws JFException {
		// check whether any TP target hit at all

		String mailBody = checkTakeProfitLevels(instrument, pPeriod, bidBar);
		if (mailBody.length() > 0) {
			mailBody = instrument.toString() + ": " + mailBody
					+ " (within bar starting at "
					+ FXUtils.getFormatedTimeCET(bidBar.getTime())
