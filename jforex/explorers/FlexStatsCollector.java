...
public class FlexStatsCollector extends BasicTAStrategy implements IStrategy {
	...
	public void onStart(IContext context) throws JFException {
		...
		flexMailGenerator.parseConfFile(conf.getProperty("flexEmailConfigFile"), conf);
	}
...
/*		if (period.equals(Period.FIVE_MINS)) {
		...
		if (period.equals(Period.THIRTY_MINS)) {
				...
				mailBody = flexMailGenerator.getMailText(instrument, period, bidBar, history, indicators, trendDetector, channelPosition, momentum, vola, tradeTrigger, conf, logLine, logDB);
				mailSubject = new String("FX pair report for " + period.toString() + " timeframe: " + instrument.toString() + " at " + FXUtils.getFormatedTimeCET(bidBar.getTime()));
