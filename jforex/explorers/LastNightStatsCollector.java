...
public class LastNightStatsCollector extends BasicTAStrategy implements
		IStrategy {
	...
	public void onBar(Instrument instrument, Period period, IBar askBar,
		...
		double ATR30min = vola.getATR(instrument, basicTimeFrame,
				OfferSide.BID, bidBar.getTime(), 14)
				* Math.pow(10, instrument.getPipScale());
		double ATR4h = vola.getATR(instrument, Period.FOUR_HOURS,
				OfferSide.BID, bidBar.getTime(), 14)
				* Math.pow(10, instrument.getPipScale());
		double ATR1d = vola.getATR(instrument, Period.DAILY_SUNDAY_IN_MONDAY,
				OfferSide.BID, prevDayTime, 14)
				* Math.pow(10, instrument.getPipScale());
		double bBandsSqueeze30min = vola.getBBandsSqueeze(instrument,
				basicTimeFrame, OfferSide.BID, bidBar.getTime(), 20);
		double bBandsSqueeze4h = vola.getBBandsSqueeze(instrument,
				Period.FOUR_HOURS, OfferSide.BID, prev4hTime, 20);
	...
	private void printOvernightReport(Instrument instrument, Period pPeriod,
	...
	protected String checkTALevels(Instrument instrument, Period pPeriod,
			IBar bidBar, String candleSignals) throws JFException {
		...
		for (SRLevel levelsToCheck : MAsToCheck) {
			if (levelsToCheck.hit(barToCheck,
				...
				if (taLevelsHit.length() == 0)
					...
					taLevelsHit += ", moving average " + levelsToCheck
							+ " hit !";
			}
		}
	...
	private List<SRLevel> getMAsValues(Instrument instrument, IBar bidBar)
			throws JFException {
		...
		result.add(new SRLevel("MA20 1d",
				indicators.sma(instrument, Period.DAILY_SUNDAY_IN_MONDAY,
						OfferSide.BID, AppliedPrice.CLOSE, 20, Filter.WEEKENDS,
						1, bidBar.getTime(), 0)[0]));
		result.add(new SRLevel("MA50 1d",
				indicators.sma(instrument, Period.DAILY_SUNDAY_IN_MONDAY,
						OfferSide.BID, AppliedPrice.CLOSE, 50, Filter.WEEKENDS,
						1, bidBar.getTime(), 0)[0]));
		result.add(new SRLevel("MA100 1d", indicators.sma(instrument,
				Period.DAILY_SUNDAY_IN_MONDAY, OfferSide.BID,
				AppliedPrice.CLOSE, 100, Filter.WEEKENDS, 1, bidBar.getTime(),
				0)[0]));
		result.add(new SRLevel("MA200 1d", indicators.sma(instrument,
				Period.DAILY_SUNDAY_IN_MONDAY, OfferSide.BID,
				AppliedPrice.CLOSE, 200, Filter.WEEKENDS, 1, bidBar.getTime(),
				0)[0]));

		return result;
	}

	protected void addMACDStats(Instrument instrument, IBar bidBar,
		...
		logLine.add(new FlexLogEntry("MACDAbs30min", new Double(momentum.MACD(
				instrument, basicTimeFrame, OfferSide.BID, AppliedPrice.CLOSE,
				bidBar.getTime())), FXUtils.df5));
		...
		logLine.add(new FlexLogEntry("MACDAbs4h", new Double(momentum.MACD(
				instrument, Period.FOUR_HOURS, OfferSide.BID,
				AppliedPrice.CLOSE, bidBar.getTime())), FXUtils.df5));
		...
		logLine.add(new FlexLogEntry("MACDAbs1d", new Double(momentum.MACD(
				instrument, Period.DAILY_SUNDAY_IN_MONDAY, OfferSide.BID,
				AppliedPrice.CLOSE, prevDayTime)), FXUtils.df5));
		...
		logLine.add(new FlexLogEntry("MACD_HAbs30min", new Double(momentum
				.MACDHistogram(instrument, basicTimeFrame, OfferSide.BID,
						AppliedPrice.CLOSE, bidBar.getTime())), FXUtils.df5));
		...
		logLine.add(new FlexLogEntry("MACD_HAbs4h", new Double(momentum
				.MACDHistogram(instrument, Period.FOUR_HOURS, OfferSide.BID,
						AppliedPrice.CLOSE, bidBar.getTime())), FXUtils.df5));
		...
		logLine.add(new FlexLogEntry("MACD_HAbs1d", new Double(momentum
				.MACDHistogram(instrument, Period.DAILY_SUNDAY_IN_MONDAY,
						OfferSide.BID, AppliedPrice.CLOSE, prevDayTime)),
				FXUtils.df5));
	}

	protected void addStochStats(Instrument instrument, IBar bidBar,
			List<FlexLogEntry> logLine) throws JFException {
		Momentum.STOCH_STATE stoch30minState = momentum.getStochState(
		...
		logLine.add(new FlexLogEntry("StochStateOverlap", stoch30minState
				...
				+ ")" : "no"));
		double[] stochs30min = momentum.getStochs(instrument, basicTimeFrame,
	...
	protected void addRSIStats(Instrument instrument, IBar bidBar,
			List<FlexLogEntry> logLine) throws JFException {
		double rsi30min = momentum.getRSI(instrument, basicTimeFrame,
	...
	protected void addFlatStats(Instrument instrument, IBar bidBar,
			List<FlexLogEntry> logLine) throws JFException {
		double trendStrength30min = trendDetector.getMAsMaxDiffStDevPos(
				instrument, basicTimeFrame, OfferSide.BID, AppliedPrice.CLOSE,
				bidBar.getTime(), FXUtils.MONTH_WORTH_OF_30min_BARS);
		...
		trendStrength30min = trendDetector.getAbsMAsDifference(instrument,
				basicTimeFrame, OfferSide.BID, AppliedPrice.CLOSE,
				bidBar.getTime());
		logLine.add(new FlexLogEntry("AbsMAsDistance30min", new Double(
				trendStrength30min), FXUtils.df1));
		...
		trendStrength4h = trendDetector.getAbsMAsDifference(instrument,
				Period.FOUR_HOURS, OfferSide.BID, AppliedPrice.CLOSE,
				bidBar.getTime());
		logLine.add(new FlexLogEntry("AbsMAsDistance4h", new Double(
				trendStrength4h), FXUtils.df1));
		logLine.add(new FlexLogEntry(
				"MAsDistanceOverlap",
		...
		trendStrength1d = trendDetector.getAbsMAsDifference(instrument,
				Period.DAILY_SUNDAY_IN_MONDAY, OfferSide.BID,
				AppliedPrice.CLOSE, prevDayTime);
		logLine.add(new FlexLogEntry("AbsMAsDistance1d", new Double(
				trendStrength1d), FXUtils.df1));

	}

	protected void addChannelPosStats(Instrument instrument, IBar bidBar,
			List<FlexLogEntry> logLine) throws JFException {
		...
		lowChannelPosPrevBar30min = channelPosition.bullishTriggerChannelStats(
				instrument, basicTimeFrame, OfferSide.BID, bidBar.getTime())[1];
		logLine.add(new FlexLogEntry("bullishTriggerLowChannelPos30min",
				new Double(lowChannelPosPrevBar30min), FXUtils.df1));
		...
		highChannelPosPrevBar30min = channelPosition
				.bearishTriggerChannelStats(instrument, basicTimeFrame,
						OfferSide.BID, bidBar.getTime())[1];
		logLine.add(new FlexLogEntry("bearishTriggerHighChannelPos30min",
				new Double(highChannelPosPrevBar30min), FXUtils.df1));

		int barsAbove = channelPosition.consequitiveBarsAbove(instrument,
				basicTimeFrame, OfferSide.BID, bidBar.getTime(), 5);
		...
		barsAbove = channelPosition.consequitiveBarsAbove(instrument,
				Period.FOUR_HOURS, OfferSide.BID, prev4hTime, 5);
		...
		barsAbove = channelPosition.consequitiveBarsAbove(instrument,
				Period.DAILY_SUNDAY_IN_MONDAY, OfferSide.BID, prevDayTime, 5);
		...
		logLine.add(new FlexLogEntry("barHigh30min", new Double(bidBar
				.getHigh()), FXUtils.df5));
		logLine.add(new FlexLogEntry("bBandsTopBarHighDiff30min", new Double(
				(bidBar.getHigh() - bBandsRaw[0])
						* Math.pow(10, instrument.getPipScale())), FXUtils.df1));
		logLine.add(new FlexLogEntry("bBandsBottomBarLow30min", new Double(
				(bBandsRaw[2] - bidBar.getLow())
						* Math.pow(10, instrument.getPipScale())), FXUtils.df1));
		logLine.add(new FlexLogEntry("bBandsTopBarHighDiff30min", new Double(
				(bidBar.getHigh() - bBandsRaw[0])
						* Math.pow(10, instrument.getPipScale())), FXUtils.df1));
	...
	protected void addCandles(Instrument instrument, IBar bidBar,
		...
		double bar30minStat = tradeTrigger.barLengthStatPos(instrument,
		...
		if ((bullishTriggerDesc = tradeTrigger
				.bullishReversalCandlePatternDesc(instrument, basicTimeFrame,
		...
		if ((bearishTriggerDesc = tradeTrigger
				.bearishReversalCandlePatternDesc(instrument, basicTimeFrame,
						OfferSide.BID, bidBar.getTime())) != null) {
	...
	private void addADXStats(Instrument instrument, IBar bidBar, Period period,
			List<FlexLogEntry> logLine) throws JFException {
		...
		logLine.add(new FlexLogEntry("DI_PLUS30min", new Double(indicators
				.plusDi(instrument, basicTimeFrame, OfferSide.BID, 14,
						Filter.WEEKENDS, 1, bidBar.getTime(), 0)[0]),
				FXUtils.df1));
		logLine.add(new FlexLogEntry("DI_MINUS30min", new Double(indicators
				.minusDi(instrument, basicTimeFrame, OfferSide.BID, 14,
						Filter.WEEKENDS, 1, bidBar.getTime(), 0)[0]),
				FXUtils.df1));
	...
	private void addCCIStats(Instrument instrument, IBar bidBar, Period period,
			List<FlexLogEntry> logLine) throws JFException {
		logLine.add(new FlexLogEntry("CCI30min", new Double(indicators.cci(
				instrument, basicTimeFrame, OfferSide.BID, 14, Filter.WEEKENDS,
	...
	protected String createReportBody4h(Instrument instrument, Period pPeriod,
			IBar bidBar, List<FlexLogEntry> logLine) throws JFException {
		...
		double roc1 = indicators.roc(instrument, Period.FOUR_HOURS,
				...
				bidBar.getTime(), 0)[0];
		mailBody += "4h price change: " + FXUtils.df1.format(roc1 * 100)
				+ " pips\n";
		...
		if (valueToShow != null && !valueToShow.toUpperCase().equals("NONE")
			...
			if (valueToShow.contains("BEARISH")) {
				candles += "pivot bar high 4h channel position: "
						+ mailStringsMap
			...
			mailBody += "\n"
					+ checkTALevels(instrument, pPeriod, bidBar,
							"BULLISH_1_BAR") + "\n";
		}
		...
		if (((Double) mailValuesMap.get("barLowChannelPos4h").getValue())
				.doubleValue() < 50.0) {
			boolean additionalSignal = false;
			if (mailStringsMap.get("MACDHState4h").equals(
		...
		} else if (((Double) mailValuesMap.get("barHighChannelPos4h")
		...
		if (mailStringsMap.get("RSIState4h").contains("TICKED_UP_FROM")
				|| mailStringsMap.get("RSIState4h")
						.contains("TICKED_DOWN_FROM"))
			mailBody += "\nRSI4h: " + mailStringsMap.get("RSI4h") + " ("
					+ mailStringsMap.get("RSIState4h") + ")";
		if (mailStringsMap.get("CCIState4h").contains("TICKED_UP_FROM")
