...
public class StratTablePlugin extends Plugin {
	...
	private static final SimpleDateFormat SEC_FORMAT = new SimpleDateFormat(
			"HH:mm:ss");
	...
	// We execute strategy actions in an executor to be able to process the
	// responses
	// without delaying neither the plugin execution nor the GUI thread.
	// Note that remote actions take much longer time to execute, for extensive
	// remote action
	...
	public void onStart(IPluginContext context) throws JFException {
		...
		localStrategyManager.addStrategyListener(new LocalStrategyListener() {
			...
			@Override
			public void onStrategyRun(
					ILocalStrategyDescriptor localStrategyDescriptor) {
				StratTablePlugin.this.onStrategyRun(localStrategyDescriptor,
						false);
			};
			...
			public void onStrategyStop(
					ILocalStrategyDescriptor localStrategyDescriptor) {
				StratTablePlugin.this.onStrategyStop(localStrategyDescriptor,
						false);
			};
		...
		remoteStrategyManager.addStrategyListener(new RemoteStrategyListener() {
			...
			@Override
			public void onStrategyRun(
					IRemoteStrategyDescriptor remoteStrategyDescriptor) {
				StratTablePlugin.this.onStrategyRun(remoteStrategyDescriptor,
						true);
			};
			...
			public void onStrategyStop(
					IRemoteStrategyDescriptor remoteStrategyDescriptor) {
				StratTablePlugin.this.onStrategyStop(remoteStrategyDescriptor,
						true);
			};
	...
	public void onStrategyRun(final IStrategyDescriptor strategyDescriptor,
			boolean isRemote) {
		(isRemote ? console.getNotif() : console.getInfo()).format(
				"%s start %s", isRemote ? "remote" : "local",
				strategyDescriptor).println();
	...
	public void onStrategyStop(final IStrategyDescriptor strategyDescriptor,
			boolean isRemote) {
		(isRemote ? console.getNotif() : console.getInfo())
				.format("%s stop %s", isRemote ? "remote" : "local",
						strategyDescriptor).println();
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
	...
	private IStrategyManager<?, ?> getManager(IStrategyDescriptor descriptor) {
		return descriptor instanceof ILocalStrategyDescriptor ? localStrategyManager
				: remoteStrategyManager;
	}

	private void placeControlsOnTab() {
		widget = context.addWidget(widgetTitle, WidgetProperties.newInstance()
		...
		table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
			...
			public Component getTableCellRendererComponent(JTable table,
				...
				c.setBackground(tableModel.isStopped(row) ? (isSelected ? Color.DARK_GRAY
						: Color.LIGHT_GRAY)
						: (isSelected ? Color.BLUE.darker().darker()
								: Color.WHITE));
				return c;
			}
		});
	...
	private JPopupMenu createPopup() {
		...
		itemStartLocal.addActionListener(new ExecutorActionListener(
				new StartSelectedAction(localStrategyManager)));
		itemStartRemote.addActionListener(new ExecutorActionListener(
				new StartSelectedAction(remoteStrategyManager)));
		itemStop.addActionListener(new ExecutorActionListener(new StopAction()));
		itemCompile.addActionListener(new ExecutorActionListener(
				new RecompileAction()));
		...
		popup.addPopupMenuListener(new PopupMenuListener() {
			...
			public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
				boolean isStoppedLocalStrategy = selected != null
						&& tableModel.isStopped(selected)
						&& selected instanceof ILocalStrategyDescriptor
						&& ((ILocalStrategyDescriptor) selected).getFile() != null;
				...
				itemStop.setEnabled(selected != null
						&& !tableModel.isStopped(selected));
			}
	...
	private JPanel createToolbar() {
		...
		btnReset.addActionListener(new ExecutorActionListener(
				new InitTableAction()));
		JButton btnStartOut = new JButton("Local Open and start");
		btnStartOut.addActionListener(new ExecutorActionListener(
				new OpenAndStartAction(localStrategyManager)));
		JButton btnStartOutRemote = new JButton("Remote Open and start");
		btnStartOutRemote.addActionListener(new ExecutorActionListener(
				new OpenAndStartAction(remoteStrategyManager)));
	...
	private class InitTableAction implements Runnable {
		...
		public void run() {
			try {
				final Set<IStrategyDescriptor> strats = (Set) localStrategyManager
						.getStartedStrategies().get().getResult();
				strats.addAll((Set) remoteStrategyManager
						.getStartedStrategies().get().getResult());
	...
	private class StartAction implements Runnable {
		...
		StartAction(IStrategyManager<?, ?> strategyManager, File file,
				Object[] params) {
			this.file = file;
			this.params = params;
		...
		public void run() {
			try {
				IStrategyResponse<UUID> startResponse = ((ILocalStrategyManager) strategyManager)
						.startStrategy(file, params).get();
				if (startResponse.isError()) {
					console.getErr().println(
							file.getName() + " strategy start failed: "
									+ startResponse.getErrorMessage());
					return;
				}
			} catch (Exception e1) {
	...
	private class OpenAndStartAction implements Runnable {
		...
		public void run() {
			...
			new StartAction(strategyManager, file, getModifiedParams(file))
					.run();
		}
	}
	...
	private class StopAction implements Runnable {
		...
		public void run() {
			try {
				IStrategyResponse<Void> startResponse = getManager(selected)
						.stopStrategy(selected.getId()).get();
				if (startResponse.isError()) {
					console.getErr().println(
							selected.getName() + " strategy stop failed: "
									+ startResponse.getErrorMessage());
					return;
				}
			} catch (Exception e1) {
	...
	private class RecompileAction implements Runnable {
		...
		public void run() {
			try {
				File file = ((ILocalStrategyDescriptor) selected).getFile();
				File srcFile = new File(file.getAbsolutePath().substring(0,
						file.getAbsolutePath().lastIndexOf(".jfx"))
						+ ".java");
				if (!srcFile.exists()) {
					console.getWarn().println(
							"can't compile - file not found - " + srcFile);
					return;
				}
	...
	// we modify the strategy parameters to our own if they are of int, double
	// or String type
	// consider here implementing some more meaningful parameter processing.
	private Object[] getModifiedParams(File file) {
		try {
			List<IStrategyParameter> defaultParams = localStrategyManager
					.getDefaultParameters(file);
			Object[] params = new Object[defaultParams.size()];
			int i = 0;
			for (IStrategyParameter p : defaultParams) {
				Class<?> type = p.getType();
				params[i++] = type.equals(int.class)
						|| type.equals(Integer.class) ? 42
						: type.equals(double.class)
								|| type.equals(Double.class) ? 42d : type
								.equals(String.class) ? "str_"
								+ SEC_FORMAT.format(System.currentTimeMillis())
								: p.getValue();
			}
			return params;
		} catch (Exception e) {
