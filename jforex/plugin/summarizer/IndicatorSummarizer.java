...
/**
 * The plugin summarizes the visible indicators on the chart by printing their
 * values, reversed values, minimums and maximums
 * 
 */
public class IndicatorSummarizer extends Plugin {
	...
	public void onStart(IPluginContext context) throws JFException {
		...
		if (chart.getIndicatorApperanceInfos().size() == 0) {
			console.getWarn()
					.println(
							"There are no indicators on the chart, nothing to summarize!");
			context.stop();
		}

		IFeedDescriptor feedDescriptor = chart.getFeedDescriptor()
				.getDataType() == DataType.TICKS ? new TimePeriodAggregationFeedDescriptor(
				chart.getFeedDescriptor().getInstrument(), Period.ONE_SEC,
				chart.getSelectedOfferSide()) : chart.getFeedDescriptor();
		...
		for (IIndicatorAppearanceInfo info : chart.getIndicatorApperanceInfos()) {
			Object[] result = indicators.calculateIndicator(feedDescriptor,
					info.getOfferSidesForTicks(), info.getName(),
					info.getAppliedPricesForCandles(), info.getOptParams(),
					count, lastElTime, 0);
			int index = 0;
			for (Object arr : result) {
				...
				long minTime = history.getFeedData(feedDescriptor,
						ArrayUtils.indexOf(indResultReversed, min)).getTime();
				long maxTime = history.getFeedData(feedDescriptor,
						ArrayUtils.indexOf(indResultReversed, max)).getTime();
				console.getOut()
						.format("%s \"%s\" for the last %s feed elements \nasc: %s \ndesc:%s "
								+ "\nmax=%.7f at %s min=%.7f at %s \n_____",
								info.getName(),
								info.getIndicator()
										.getOutputParameterInfo(index)
										.getName(), indResult.length,
								toString(indResult),
								toString(indResultReversed), max,
								DateUtils.format(maxTime), min,
								DateUtils.format(minTime)).println();
				index++;
			}
		}
		context.stop();
