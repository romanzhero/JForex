...
class OrderTableModel extends AbstractTableModel {
	...
	private static double round(double amount, int decimalPlaces) {
		return (new BigDecimal(amount)).setScale(decimalPlaces,
	...
	private final OrderColumn[] orderColumns = new OrderColumn[] {
			OrderColumn.newReadOnlyColumn("Order label", new IOrderGet() {
				@Override
				public String getValue(IOrder order) {
					return order.getLabel();
				}
			}),
			OrderColumn.newReadOnlyColumn("Instrument", new IOrderGet() {
				@Override
				public String getValue(IOrder order) {
					return order.getInstrument().toString();
				}
			}),
			OrderColumn.newEditableColumn("Stop Loss", new IOrderGet() {
				@Override
				public String getValue(IOrder order) {
					return order.getStopLossPrice() == 0 ? "-" : String.format(
							"%.6f", order.getStopLossPrice());
				}
			}, new IOrderSet() {
				@Override
				public void setValue(IOrder order, Object value)
						throws JFException {
					order.setStopLossPrice(round(Double.valueOf(value
							.toString()),
							order.getInstrument().getPipScale() + 1));
				}
			}),
			OrderColumn.newEditableColumn("Take Profit", new IOrderGet() {
				@Override
				public String getValue(IOrder order) {
					return order.getTakeProfitPrice() == 0 ? "-" : String
							.format("%.6f", order.getTakeProfitPrice());
				}
			}, new IOrderSet() {
				@Override
				public void setValue(IOrder order, Object value)
						throws JFException {
					order.setTakeProfitPrice(round(Double.valueOf(value
							.toString()),
							order.getInstrument().getPipScale() + 1));
				}
			}),
			OrderColumn.newEditableColumn("SL distance in pips",
					new IOrderGet() {
						@Override
						public String getValue(IOrder order) {
							...
							double lastPrice = getLastPrice(
									order.getInstrument(), order.isLong());
						...
						public void setValue(IOrder order, Object value)
							...
							double lastPrice = getLastPrice(
									order.getInstrument(), order.isLong());
							double priceDelta = Double.valueOf(value.toString())
									* order.getInstrument().getPipValue();
							double price = order.isLong() ? lastPrice
									- priceDelta : lastPrice + priceDelta;
							order.setStopLossPrice(round(price, order
									.getInstrument().getPipScale() + 1));
						}
					}),
			OrderColumn.newEditableColumn("TP distance in pips",
					new IOrderGet() {
						@Override
						public String getValue(IOrder order) {
						...
						public void setValue(IOrder order, Object value)
							...
							double price = order.isLong() ? lastPrice
									+ priceDelta : lastPrice - priceDelta;
							order.setTakeProfitPrice(round(price, order
									.getInstrument().getPipScale() + 1));

						}
					}),
			OrderColumn.newEditableColumn("Order notes", new IOrderGet() {
				@Override
				public String getValue(IOrder order) {
	...
	public void setValueAt(final Object aValue, int rowIndex,
		...
		context.executeTask(new Callable<Object>() {
			...
			public Object call() throws Exception {
				try {
					orderColumns[columnIndex].getOrderSet().setValue(order,
							aValue);
				} catch (JFException e) {
