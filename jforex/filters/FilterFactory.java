...
public class FilterFactory {
    ...
    static {
        ...
        simpleFiltersMap.put("UptrendMAsDistance", new UptrendMAsDistanceFilter());
        simpleFiltersMap.put("EntryChannelPos", new ChannelPosFilter());
        simpleFiltersMap.put("StopChannelPos", new ChannelPosFilter());
        simpleFiltersMap.put("DowntrendMAsDistance", new DowntrendMAsDistanceFilter());
        simpleFiltersMap.put("StochFast", new StochFastFilter());
        simpleFiltersMap.put("ADXDiPlus", new ADXDiPlusFilter());
    }    

    static {
        conditionalFiltersMap.put("MACDSignal_MACDsPosition", new ConditionalFilterMACDSignalMACDsPosition());
        conditionalFiltersMap.put("MACDHistogram_MACDsPosition", new ConditionalFilterMACD_HMACDsPosition());
        conditionalFiltersMap.put("StochsDiff_MACDsPosition", new ConditionalFilterStochsDiffMACDsPosition());
        conditionalFiltersMap.put("StochSlow_MACDsPosition", new ConditionalFilterStochSlowMACDsPosition());
        conditionalFiltersMap.put("ADX_MAsPosition", new ConditionalFilterADXMAsPosition());
        conditionalFiltersMap.put("UptrendMAsDistance_MAsPosition", new ConditionalFilterUptrendMAsDistanceMAsPosition());        
        conditionalFiltersMap.put("ChannelPos_MAsPosition", new ConditionalFilterChannelPosMAsPosition());        
        conditionalFiltersMap.put("StochsDiff_MAsPosition", new ConditionalFilterStochsDiffMAsPosition());        
        conditionalFiltersMap.put("MACDSignal_MAsPosition", new ConditionalFilterMACDSignalMAsPosition());        
        conditionalFiltersMap.put("MACDHistogram_MAsPosition", new ConditionalFilterMACD_HMAsPosition());        
        conditionalFiltersMap.put("DowntrendMAsDistance_MAsPosition", new ConditionalFilterDowntrendMAsDistanceMAsPosition());        
        conditionalFiltersMap.put("StochSlow_MAsPosition", new ConditionalFilterStochSlowMAsPosition());        
        conditionalFiltersMap.put("StochFast_MAsPosition", new ConditionalFilterStochFastMAsPosition());        
        conditionalFiltersMap.put("ADXDiPlus_MAsPosition", new ConditionalFilterADXDiPlusMAsPosition());
        conditionalFiltersMap.put("ADXDiPlus_ADXsPosition", new ConditionalFilterADXDiPlusADXsPosition());
        conditionalFiltersMap.put("ChannelPos_ADX", new ConditionalFilterChannelPosADX());        
    }    

    static public IFilter createFilter(String filterName, String filterValue, IIndicators pIndicators, IHistory pHistory)
	{
		if (!filterName.startsWith(FILTER_PREFIX) && !filterName.startsWith(FILTER_CONDITIONAL_PREFIX))
			return null;
		if (filterName.startsWith(FILTER_CONDITIONAL_PREFIX)) {
			...
			if (conditionalFiltersMap.containsKey(indicatorCombination)) {
				...
				IConditionalFilter f = conditionalFiltersMap.get(indicatorCombination).cloneConditionalFilter();
				f.set(filterName, pIndicators, mainPeriod, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
				f.setConditionalParams(pHistory, condPeriod, mainMin, mainMax, condMin, condMax);
				return f;
			}
			else
				return null;
	...
	// <cond min 1>;<cond max 1>;<min 1>;<max 1>;<cond min 2>;<cond max 2>;<min 2>;<max 2>;...;<cond min n>;<cond max n>;<min n>;<max n>
    ...
    private static double[] parseCondMax(String filterValue) {
		...
		int totalTokens = st.countTokens(); // mod 4 must be 0 if all syntax rules were kept
		if (totalTokens % 4 != 0)
			return null;
	...
	private static double[] parseCondMin(String filterValue) {
		StringTokenizer st = new StringTokenizer(filterValue, VALUE_DELIMITER);
		int totalTokens = st.countTokens(); // mod 4 must be either 0 (complete conditions) or 1 (last max empty)
		if (totalTokens % 4 != 0)
			return null;
		int totalFours = totalTokens / 4;
		double[] result = new double[totalFours];
	...
	// conditional filters have format FILTER_CONDITIONAL_<time frame>_<main indicator>_<conditional indicator>_<conditional time frame>
	// = <cond min 1>;<cond max 1>;<min 1>;<max 1>;<cond min 2>;<cond max 2>;<min 2>;<max 2>;...;<cond min n>;<cond max n>;<min n>;<max n>
	...
	private static double parseMax(String filterValue) {
		// Format of simple filters' values is <NEGATIVE_INFINITY;<POSITIVE_INFINITY>
		StringTokenizer st = new StringTokenizer(filterValue, VALUE_DELIMITER);
		st.nextToken();
		String token = st.nextToken();
		if (token.equals("NONE"))
