...
public abstract class AbstractConditionalFilter extends AbstractSimpleFilter {
	...
	public void setConditionalParams(IHistory pHistory, Period condPeriod,
			double[] pMainMin, double[] pMainMax, double[] pCondMin,
			double[] pCondMax) {
	...
	protected abstract double calcCondFilterValue(Instrument instrument,
	...
	public boolean check(Instrument instrument, OfferSide side,
		...
		for (int i = 0; i < mainMin.length; i++) {
			...
			if (condFilterValue >= conditionMin[i]
				...
				lastCondFilter = "[" + FXUtils.df2.format(conditionMin[i])
						+ ", " + FXUtils.df2.format(conditionMax[i]) + "]";
				// for the rest simply delegate to simple filter
				mainFilter.set("Main filter of " + name, indicators, period,
						mainMin[i], mainMax[i]);
				mainFilter.setAuxParams(auxParams);
				return mainFilter.check(instrument, side, appliedPrice,
						mainTime);
			}
		}
	...
	public String explain(long barTime) {
		return mainFilter.explain(barTime) + "; cond. filter interval = "
				+ lastCondFilter;
	}
