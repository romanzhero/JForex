...
public abstract class AbstractSimpleFilter {
	...
	protected abstract double calcIndicator(Instrument instrument, OfferSide side,	AppliedPrice appliedPrice, long time) throws JFException;
	...
	public boolean check(Instrument instrument, OfferSide side, AppliedPrice appliedPrice, long time) throws JFException {
		double indicatorValue = calcIndicator(instrument, side, appliedPrice, time);
		if (indicatorValue >= minAllowedValue && indicatorValue <= maxAllowedValue) {
			...
			explanation = new String(name + ";" + instrument.toString() + ";"
					...
					+ ";filtered out: " 
					+ decFormat().format(indicatorValue) + " not within [" + decFormat().format(minAllowedValue) + ", "  + decFormat().format(maxAllowedValue) + "]");
			return false;
		}
	}

	public String explain(long barTime) { return "For bar: " + FXUtils.getFormatedTimeGMT(barTime) + ";" + explanation; }
