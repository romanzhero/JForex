...
public class ConditionalFilterChannelPosADX extends ConditionalFilterOnADX implements IConditionalFilter {
	...
	public boolean check(Instrument instrument, OfferSide side, AppliedPrice appliedPrice, long time) throws JFException {
		...
		for (int i = 0; i < mainMin.length; i++) {
			...
			if (condFilterValue >= conditionMin[i] && condFilterValue <= conditionMax[i]) {
				...
				//double[] last2BarsChannelPos = ch.bullishTriggerChannelStats(instrument, period, side, mainTime);
//				if (last2BarsChannelPos[1] >= mainMax[i])
//				return false;
				double triggerBarLowChannelPos = ch.priceChannelPos(instrument, period, side, (long) auxParams[0], auxParams[1]);
