...
public class AbstTradeLog {
	private int order_id, position_log_id;

	private double maxPnL, maxPnl_pips, maxPnL_bc, maxPnL_pips_bc,
	...
	public AbstTradeLog(int order_id, int position_log_id, IOrder order,
			Connection pLogDB) {
		super();
		...
		min_tp_distance_pips = Math.abs(order.getTakeProfitPrice()
				- order.getOpenPrice());
	...
	public void dbRecordFill(long logTime) {
		FXUtils.dbUpdateInsert(
				...
				"INSERT INTO "
						+ FXUtils.getDbToUse()
						+ ".ttradelog(order_id, positionlog_id, log_time, event) VALUES ("
	...
	public void update(IBar bidBar, IBar askBar, long logTime) {
		if (order.isLong()) {
			double tpDistancePips = order.getTakeProfitPrice()
					- bidBar.getHigh();
			...
			double tpDistancePips = askBar.getLow()
