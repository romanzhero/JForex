...
public abstract class AbstractMultiPositionStrategy extends AbstractTradingStrategy {
	...
	public AbstractMultiPositionStrategy(Properties props) {
		super(props);
		MAX_POSITIONS = Integer.parseInt(props.getProperty("MAX_POSITIONS", "4"));
	}
	
	protected void placeBuyOrder(Instrument instrument, Period period, IBar askBar,	IBar bidBar) throws JFException {
		double entryPrice = bidBar.getHigh() + safeZone / Math.pow(10, instrument.getPipScale());
		// adjust stop loss to be below 35 pips
		if (orders.size() == 0 && (entryPrice - protectiveStop) * Math.pow(10, instrument.getPipScale()) > MAX_RISK)
		{
			protectiveStop = entryPrice - MAX_RISK / Math.pow(10, instrument.getPipScale());
		}
		
		double tradeCandleLeverage = 1;
		if (FLEXIBLE_LEVERAGE) {
			tradeCandleLeverage = MAX_RISK / (entryPrice - protectiveStop) / Math.pow(10, instrument.getPipScale()); 
		}
	    ...
	    waitingOrder  = engine.submitOrder(getLabel(instrument), instrument, OrderCommand.BUYSTOP, tradeAmount, entryPrice);
	}
	
	protected void placeBuyOrder(Instrument instrument, Period period, IBar askBar,	IBar bidBar, double stopLossLevel) throws JFException {
		double entryPrice = bidBar.getHigh() + safeZone / Math.pow(10, instrument.getPipScale());
	...
	protected void placeSellOrder(Instrument instrument, Period period, IBar askBar, IBar bidBar) throws JFException {
		double entryPrice = bidBar.getLow() - safeZone / Math.pow(10, instrument.getPipScale());
		// adjust stop loss to be below 35 pips
		if (orders.size() == 0 && (protectiveStop - entryPrice) * Math.pow(10, instrument.getPipScale()) > MAX_RISK)
		{
			protectiveStop = entryPrice + MAX_RISK / Math.pow(10, instrument.getPipScale());
		}
		...
		waitingOrder = engine.submitOrder(getLabel(instrument), instrument, OrderCommand.SELLSTOP, tradeAmount, entryPrice);
	}

	protected void placeSellOrder(Instrument instrument, Period period, IBar askBar, IBar bidBar, double stopLoss) throws JFException {
		double entryPrice = bidBar.getLow() - safeZone / Math.pow(10, instrument.getPipScale());
	...
	protected void processOrderFill(IMessage message) {
		...
		if (orders.size() < MAX_POSITIONS)
			...
			tradeState.stateTransition(TradeState.POSITION_OPENED_MAX_REACHED, message.getCreationTime());
		log.printAction(Logger.logTags.ORDER.toString(), 
				...
				(protectiveStop - waitingOrder.getOpenPrice()) * Math.pow(10, message.getOrder().getInstrument().getPipScale()), 
				0, 0);
	...
	protected void processOrderFill(IMessage message, boolean phaseTwo) {
		...
		orders.add(waitingOrder);
		if (orders.size() < MAX_POSITIONS)
			tradeState.stateTransition(phaseTwo ? TradeState.POSITION_N_OPENED : TradeState.POSITION_OPENED, message.getCreationTime());
		else
			tradeState.stateTransition(TradeState.POSITION_OPENED_MAX_REACHED, message.getCreationTime());
		log.printAction(Logger.logTags.ORDER.toString(), 
				waitingOrder.getLabel(), 
	...
	protected void processPositionsCloseBare(IMessage message, boolean trailing) throws JFException {
		log.printAction(Logger.logTags.ORDER.toString(), 
				...
				(protectiveStop - message.getOrder().getOpenPrice()) * Math.pow(10, message.getOrder().getInstrument().getPipScale()), 
