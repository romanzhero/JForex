...
public class MultiDDLog {
	...
	protected int 
		noOfLevels,
		levelMultiplier = 2, // how many ATRs are counted for new level 
		currLevel = 0;
	...
	public void updateLevel(int level, double maxDD, double atr, double atrEMA, double atrSMA, long DDtime) {
	...
	// generic call to start internal calculations. maxProfit must be already calculated !!
	public void update(IBar currBar, double fillPrice, double maxProfitPrice, long maxProfitTime, double atr, double atrEMA, double atrSMA) {
		// establish in which level to work
		int tempLevel = new Double(Math.floor((isLong ? currBar.getHigh() - fillPrice : fillPrice - currBar.getLow()) / (levelMultiplier * atr))).intValue();
		if (tempLevel >= currLevel
			&& tempLevel < noOfLevels
...
//            return;
        ...
        if (isLong) {
            if (maxProfitPrice - currBar.getLow() > levelRecords[currLevel].maxDD) {
            	levelRecords[currLevel].maxDD = maxProfitPrice - currBar.getLow();
            ...
            if (currBar.getHigh() - maxProfitPrice > levelRecords[currLevel].maxDD) {
            	levelRecords[currLevel].maxDD = currBar.getHigh() - maxProfitPrice;
            	levelRecords[currLevel].DDtime = currBar.getTime();
            	levelRecords[currLevel].atr = atr;
            	levelRecords[currLevel].atrEMA = atrEMA;
            	levelRecords[currLevel].atrSMA = atrSMA;
	...
	public LevelLog getLevel(int level) { return levelRecords[level]; }
