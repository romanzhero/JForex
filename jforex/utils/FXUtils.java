...
public class FXUtils {
	...
	public static final int QUARTER_WORTH_OF_4h_BARS = 380; // TODO: too little
															// compared to
															// others ???
	...
	public enum EntryFilters {
		FILTER_MOMENTUM, FILTER_ENTRY_BAR_HIGH, FILTER_ENTRY_BAR_LOW, FILTER_4H
	}
	...
	static {
		timeFrameStatsMap.put(Period.TEN_MINS.toString(), new Integer(
				TWO_WEEKS_WORTH_OF_10min_BARS));
		timeFrameStatsMap.put(Period.FIFTEEN_MINS.toString(), new Integer(
				TWO_WEEKS_WORTH_OF_15min_BARS));
		timeFrameStatsMap.put(Period.TWENTY_MINS.toString(), new Integer(
				TWO_WEEKS_WORTH_OF_20min_BARS));
		timeFrameStatsMap.put(Period.THIRTY_MINS.toString(), new Integer(
				MONTH_WORTH_OF_30min_BARS));
		timeFrameStatsMap.put(Period.ONE_HOUR.toString(), new Integer(
				MONTH_WORTH_OF_1h_BARS));
		timeFrameStatsMap.put(Period.FOUR_HOURS.toString(), new Integer(
				QUARTER_WORTH_OF_4h_BARS));
		timeFrameStatsMap.put(Period.DAILY_SUNDAY_IN_MONDAY.toString(),
				new Integer(YEAR_WORTH_OF_1d_BARS));
		timeFrameStatsMap.put(Period.WEEKLY.toString(), new Integer(
				TWO_YEAR_WORTH_OF_1W_BARS));
	}

	public static Map<String, String> timeFramesHigherTFMap = new HashMap<String, String>();
	static {
		timeFramesHigherTFMap.put(Period.THIRTY_MINS.toString(),
				Period.FOUR_HOURS.toString());
		timeFramesHigherTFMap.put(Period.FOUR_HOURS.toString(),
				Period.DAILY_SUNDAY_IN_MONDAY.toString());
		timeFramesHigherTFMap.put(Period.DAILY_SUNDAY_IN_MONDAY.toString(),
	...
	static {
		...
		timeFramesHigherTFPeriodMap.put(Period.FOUR_HOURS,
				Period.DAILY_SUNDAY_IN_MONDAY);
		timeFramesHigherTFPeriodMap.put(Period.DAILY_SUNDAY_IN_MONDAY,
				Period.WEEKLY);
	}
	...
	public static BigDecimal[] calculateProfitLossData(IOrder order)
			throws JFException {
		...
		if (order.getOrderCommand().isLong()) {
			...
			res[2] = BigDecimal
					...
					.subtract(BigDecimal.valueOf(order.getOpenPrice()))
					.multiply(
							BigDecimal.valueOf(10).pow(
									order.getInstrument().getPipScale()))
		...
		OfferSide side = order.getOrderCommand().isLong() ? OfferSide.ASK
				: OfferSide.BID;
		res[1] = convertByTick(profLossInSecondaryCCY,
	...
	public static BigDecimal convertByTick(BigDecimal amount, Currency sourceCurrency, Currency targetCurrency, OfferSide side)
		...
		if (sourceCurrency.equals(Instrument.EURUSD.getSecondaryCurrency())) {
			...
			BigDecimal helperSourceCurrencyPrice = getLastTickPrice(
					helperSourceCurrencyPair, side);
			if (null == helperSourceCurrencyPrice)
				return null;

			dollarValue = helperSourceCurrencyPair.toString().indexOf("USD") == 0 ? amount
					.divide(helperSourceCurrencyPrice, 2,
							RoundingMode.HALF_EVEN) : amount.multiply(
					helperSourceCurrencyPrice).setScale(2,
		...
		BigDecimal result = pair.toString().indexOf("USD") == 0 ? dollarValue
				.multiply(price).setScale(2, RoundingMode.HALF_EVEN)
				: dollarValue.divide(price, 2, RoundingMode.HALF_EVEN);

		return result;
	}

	protected static BigDecimal getLastTickPrice(Instrument pair, OfferSide side)
			throws JFException {
	...
	public static BigDecimal convertByBar(BigDecimal amount, Currency sourceCurrency, Currency targetCurrency, Period period, OfferSide side, long time) throws JFException {
		...
		if (sourceCurrency.equals(Instrument.EURUSD.getSecondaryCurrency())) {
			...
			BigDecimal helperSourceCurrencyPrice = getLastBarPrice(
					helperSourceCurrencyPair, side, period, time);
			if (null == helperSourceCurrencyPrice) {
	...
	public static DateTime calcEndBar() {
		...
		DateTime timeStamp = new DateTime(), roundedEnd = new DateTime(
				timeStamp.getYear(), timeStamp.getMonthOfYear(),
				timeStamp.getDayOfMonth(), timeStamp.getHourOfDay(),
				timeStamp.getMinuteOfHour() < 30 ? 0 : 30, 0, 0), result = new DateTime(
				roundedEnd);
		if ((roundedEnd.getDayOfWeek() == DateTimeConstants.FRIDAY && roundedEnd
				.getHourOfDay() > 22)
				|| roundedEnd.getDayOfWeek() == DateTimeConstants.SATURDAY
				|| (roundedEnd.getDayOfWeek() == DateTimeConstants.SUNDAY && roundedEnd
						.getHourOfDay() < 23)) {
			if (roundedEnd.getDayOfWeek() == DateTimeConstants.FRIDAY)
				result = new DateTime(roundedEnd.getYear(),
						roundedEnd.getMonthOfYear(),
						roundedEnd.getDayOfMonth(), 22, 30, 0, 0);
			else if (roundedEnd.getDayOfWeek() == DateTimeConstants.SATURDAY) {
				DateTime friday = new DateTime(roundedEnd.minusDays(1));
				result = new DateTime(friday.getYear(),
						friday.getMonthOfYear(), friday.getDayOfMonth(), 22,
						30, 0, 0);
	...
	public static DateTime calcTimeOfLastNYClose30minBar(long time) {
		// During weekend set end bar to last trading bar of previous Friday
		DateTime timeStamp = new DateTime(time), roundedEnd = new DateTime(
				timeStamp.getYear(), timeStamp.getMonthOfYear(),
				timeStamp.getDayOfMonth(), timeStamp.getHourOfDay(),
				timeStamp.getMinuteOfHour() < 30 ? 0 : 30, 0, 0), result = new DateTime(
				roundedEnd);
		if ((roundedEnd.getDayOfWeek() == DateTimeConstants.FRIDAY && roundedEnd
				...
				|| (roundedEnd.getDayOfWeek() == DateTimeConstants.MONDAY && (roundedEnd
						.getHourOfDay() < 22 || (roundedEnd.getHourOfDay() == 22 && roundedEnd
						.getMinuteOfHour() < 30)))) {
			// anything after last close in the week and on Sunday --> put to
			// last Friday's close !
			if (roundedEnd.getDayOfWeek() == DateTimeConstants.FRIDAY)
				result = new DateTime(roundedEnd.getYear(),
						roundedEnd.getMonthOfYear(),
			...
			if (roundedEnd.getHourOfDay() >= 0) {
				...
				result = new DateTime(prevDay.getYear(),
						prevDay.getMonthOfYear(), prevDay.getDayOfMonth(), 22,
						30, 0, 0);
			} else {
				result = new DateTime(roundedEnd.getYear(),
						roundedEnd.getMonthOfYear(),
	...
	public static DateTime getLastTradingDay(long time) {
		DateTime current = new DateTime(time, DateTimeZone.forID("UTC")), result = null;
		...
		return new DateTime(result.getYear(), result.getMonthOfYear(),
				result.getDayOfMonth(), 0, 0, 0, 0, DateTimeZone.forID("UTC"));
	}

	/**
	 * Accounts for weekends. For trades made during Fri next allowed trade is
	 * calculated from Sun 23h CET
	 */
	public static boolean tradingDistanceOK(long currentTime,
			long nextTradeAllowedTime) {
		DateTime next = new DateTime(nextTradeAllowedTime,
		...
		// nextTradeAllowedTime falls in market close hours over weekend. Need
		// adjustment for difference between Fri market close and Sun market
		// open
	...
	/**
	 * Adds time but accounts for weekends, so more is added if original
	 * currentTime + timeToAdd falls into weekend
	 */
	public static long plusTradingTime(long currentTime, long timeToAdd) {
		DateTime next = new DateTime(currentTime + timeToAdd,
	...
	public static String replaceCentered(String template, String replacement) {
		int replPos = (template.length() - replacement.length()) / 2, restPos = replPos
				+ replacement.length();
		if (replPos > 0 && restPos > 0)
			return new String(template.substring(0, replPos) + replacement
					+ template.substring(restPos));
		else
			return new String("Can not convert ! template = " + template
					+ ", replacement = " + replacement + " (replPos = "
					+ replPos + ", restPos = " + restPos);
	}

	public static String beautify(String s) {
		String lower = s.toLowerCase().replace('_', ' ').replace("zero", "0");
		return new String(lower.substring(0, 1).toUpperCase()
				+ lower.substring(1));
	}
	...
	public static ResultSet dbGetAllSubscribedInstruments(Properties properties) {
		try {
			...
			System.out
					.print("ODBC driver load failure (ClassNotFoundException: "
							+ e1.getMessage() + ")");
			System.exit(1);
		}
		...
		try {
			...
			System.out.print("Log database problem: " + ex.getMessage()
					+ " when trying to establish connection");
			System.exit(1);
		}

		String statementStr = new String(
				"SELECT distinct ticker as ticker FROM " + FXUtils.getDbToUse()
						+ ".vsubscriptions");
		try {
	...
	public static ResultSet dbGetTestTrades(Properties properties) {
		...
		String statementStr = new String(
				"SELECT Label, Event, EventStart, EventEnd FROM "
						+ FXUtils.getDbToUse()
						+ ".ttesttrades WHERE testRunDone = 'N'");
		try {
			Statement qry = logDB.createStatement();
			ResultSet result = qry.executeQuery(statementStr);
			return result;
	...
	public static ResultSet dbCountTestTrades(Properties properties) {
		...
		String statementStr = new String("SELECT count(*) as cnt FROM "
				+ FXUtils.getDbToUse() + ".ttesttrades WHERE testRunDone = 'N'");
		try {
			Statement qry = logDB.createStatement();
			ResultSet result = qry.executeQuery(statementStr);
			return result;
	...
	public static void dbUpdateInsert(Connection logDB, String sql) {
		try {
			...
			System.out.println("Problem when inserting into the DB: "
					+ ex.getMessage());
			System.out.println(sql);
			try {
				System.setErr(new PrintStream(new FileOutputStream(
						"d:\\temp\\system.err.txt")));
			} catch (FileNotFoundException e) {
				e.printStackTrace();
			}
			System.err.println("Problem when inserting into the DB: "
					+ ex.getMessage());
	...
	public static String dbGetTradeLogInsert(List<FlexLogEntry> fields,
			String bt_run, String order_label, String direction, String event) {
		String dbFieldNamesStr = new String(
				" (bt_run, order_label, direction, event"), dbValuesStr = new String(
				" VALUES ('" + bt_run + "', '" + order_label + "', '"
						+ direction + "', '" + event + "'");
		...
		dbFieldNamesStr += ") ";
		dbValuesStr += ")";
		String statementStr = "INSERT INTO " + dbToUse + ".tabstradelog "
				+ dbFieldNamesStr + dbValuesStr;
	...
	public static void dbExecSQL(Connection logDB, String sql) {
		try {
			...
			System.out
					.println("Problem when executing SQL: " + ex.getMessage());
			System.out.println(sql);
			System.err
					.println("Problem when executing SQL: " + ex.getMessage());
			System.err.println(sql);
	...
	public static ResultSet dbReadQuery(Connection logDB, String sql) {
		try {
			...
			System.out.println("Problem when reading from the DB: "
					+ ex.getMessage());
			System.out.println(sql);
			System.exit(1);
		}
	...
	public static int dbGetInstrumentID(Connection logDB, String ticker) {
		...
		String statementStr = new String("SELECT instrument_id FROM "
				+ FXUtils.getDbToUse() + ".tinstrument WHERE ticker = '"
				+ ticker + "'");
		try {
			Statement qry = logDB.createStatement();
			ResultSet result = qry.executeQuery(statementStr);
	...
	public static int dbGetUserID(Connection logDB, String userID) {
		...
		String statementStr = new String("SELECT user_id FROM "
				+ FXUtils.getDbToUse() + ".tuser where email = '" + userID
				+ "'");
		try {
			Statement qry = logDB.createStatement();
			ResultSet result = qry.executeQuery(statementStr);
	...
	public static String httpGet(String urlToCall) {
		...
		try {
			...
			result = in.readLine();
			/*
			 * old conn = (HttpURLConnection) url.openConnection();
			 * conn.setRequestMethod("GET"); rd = new BufferedReader(new
			 * InputStreamReader(conn.getInputStream())); while ((line =
			 * rd.readLine()) != null) { result += line; } rd.close();
			 */
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static int toTimeFrameCode(Period pPeriod, Connection logDB,
			Logger log) {
		String statementStr = new String("SELECT code_value FROM "
				+ FXUtils.getDbToUse()
				+ ".tcode WHERE code_group = 'TimeFrame' AND code_desc = '"
	...
	public static Period fromTimeFrameCode(int timeFrameCode, Connection logDB,
			Logger log) {
		String statementStr = new String("SELECT code_desc FROM "
				+ FXUtils.getDbToUse()
		...
		try {
			...
			log.print("Log database problem fetching time frame name: "
					+ ex.getMessage());
			log.print(statementStr);
			log.close();
			System.exit(1);
	...
	public static class TradeLog {
		...
		public long signalTime, fillTime, maxProfitTime, maxLossTime,
				maxDDTime, exitTime;
		public double entryPrice, fillPrice, SL, initialRisk, ratioCloudATR,
				maxRisk, maxLoss, maxLossATR, maxDD, maxDDATR, maxProfit,
				maxProfitPrice, PnL;

		public String exitReason = null;

		public TradeLog(String pOrderLabel, boolean pIsLong, long pSignalTime,
				double pEntryPrice, double pSL, double pInitialRisk) {
		...
		public double missedProfitPerc(Instrument instrument) {
			// PnL taken from IOrder and already in pips
			return PnL > 0.0 && maxProfit != 0.0 ? 100
					* missedProfit(instrument)
		...
		public String exitReport(Instrument instrument) {
			return new String(
					"ER;"
							...
							+ exitReason
							+ ";"
							+ (instrument.getPipScale() == 2 ? df2
									.format(entryPrice) : df5
									.format(entryPrice))
							+ ";"
							+ (instrument.getPipScale() == 2 ? df2
									.format(fillPrice) : df5.format(fillPrice))
							+ ";"
							+ (instrument.getPipScale() == 2 ? df2.format(SL)
									: df5.format(SL))
							+ ";"
							+ df1.format(initialRisk
							...
							+ ";"
							+ df1.format(maxProfit
									* Math.pow(10, instrument.getPipScale()))
							+ ";"
							+ (instrument.getPipScale() != 2 ? df5
									.format(maxProfitPrice) : df2
									.format(maxProfitPrice)) + ";"
		...
		public List<FlexLogEntry> exportToFlexLogs(Instrument instrument,
			...
			l.add(new FlexLogEntry("signalTime", FXUtils
					.getFormatedTimeGMT(signalTime)));
			l.add(new FlexLogEntry("fillTime", FXUtils
					.getFormatedTimeGMT(fillTime)));
			l.add(new FlexLogEntry("exitTime", FXUtils
					.getFormatedTimeGMT(exitTime)));
			l.add(new FlexLogEntry("exitReason", exitReason));
			l.add(new FlexLogEntry("entryPrice", new Double(entryPrice),
					instrument.getPipScale() == 2 ? FXUtils.df2 : FXUtils.df5));
			l.add(new FlexLogEntry("fillPrice", new Double(fillPrice),
					instrument.getPipScale() == 2 ? FXUtils.df2 : FXUtils.df5));
			l.add(new FlexLogEntry("SL", new Double(SL), instrument
					.getPipScale() == 2 ? FXUtils.df2 : FXUtils.df5));
			l.add(new FlexLogEntry("initialRisk", new Double(initialRisk
					* Math.pow(10, instrument.getPipScale())), FXUtils.df1));
			l.add(new FlexLogEntry("maxRisk", new Double(maxRisk
					* Math.pow(10, instrument.getPipScale())), FXUtils.df1));
			l.add(new FlexLogEntry("maxLoss", new Double(maxLoss
					* Math.pow(10, instrument.getPipScale())), FXUtils.df1));
			l.add(new FlexLogEntry("maxLossATR", new Double(maxLossATR),
					FXUtils.df1));
			l.add(new FlexLogEntry("maxLossTime", FXUtils
					.getFormatedTimeGMT(maxLossTime)));
			l.add(new FlexLogEntry("maxDD", new Double(maxDD
					* Math.pow(10, instrument.getPipScale())), FXUtils.df1));
			l.add(new FlexLogEntry("maxDDATR", new Double(maxDDATR),
					FXUtils.df1));
			l.add(new FlexLogEntry("maxDDTime", FXUtils
					.getFormatedTimeGMT(maxDDTime)));
			l.add(new FlexLogEntry("maxProfit", new Double(maxProfit
					* Math.pow(10, instrument.getPipScale())), FXUtils.df1));
			l.add(new FlexLogEntry("maxProfitPrice",
					new Double(maxProfitPrice),
					instrument.getPipScale() == 2 ? FXUtils.df2 : FXUtils.df5));
			l.add(new FlexLogEntry("maxProfitTime", FXUtils
					.getFormatedTimeGMT(maxProfitTime)));
			l.add(new FlexLogEntry("PnL", new Double(PnL), FXUtils.df1));
			l.add(new FlexLogEntry("missedProfit", new Double(
					missedProfit(instrument)), FXUtils.df1));
			l.add(new FlexLogEntry("missedProfitPerc", new Double(
					missedProfitPerc(instrument)), FXUtils.df1));
			return l;
		}
	}
