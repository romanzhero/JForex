	...
	public void validate(Logger pLog) {
		...
		if (!containsKey("cachedir")) {
			...
			if (!cachedir.isDirectory() || !cachedir.canRead()
					|| !cachedir.canWrite()) {
				pLog.error("Cache directory "
						+ getProperty("cachedir")
		...
		if (!containsKey("MAX_RISK")) {
			...
			try {
				...
				pLog.error("Format of maximum risk wrong, must be integer: "
						+ getProperty("MAX_RISK") + ", exception "
						+ e.getMessage());
				System.exit(1);
			}
		}
		...
		if (!containsKey("BREAK_EVEN_PROFIT_THRESHOLD")) {
			...
			try {
				...
				pLog.error("Format of breakeven profit level wrong, must be integer: "
						+ getProperty("BREAK_EVEN_PROFIT_THRESHOLD")
		...
		if (!containsKey("PROTECT_PROFIT_THRESHOLD")) {
			...
			try {
				...
				pLog.error("Format of profit protection level wrong, must be integer: "
						+ getProperty("PROTECT_PROFIT_THRESHOLD")
						+ ", exception " + e.getMessage());
				System.exit(1);
			}
		}
		if (!containsKey("PROTECT_PROFIT_THRESHOLD_OFFSET")) {
			...
			try {
				...
				pLog.error("Format of profit protection offset wrong, must be integer: "
						+ getProperty("PROTECT_PROFIT_THRESHOLD_OFFSET")
						+ ", exception " + e.getMessage());
				System.exit(1);
			}
		}
		...
		if (!containsKey("trendStDevDefinitionMin")) {
			...
			try {
				...
				pLog.error("Format of trend StDev definition minimum wrong, must be decimal number: "
		...
		if (!containsKey("trendStDevDefinitionMax")) {
			...
			try {
				...
				pLog.error("Format of trend StDev definition maximum wrong, must be decimal number: "
		...
		if (!containsKey("safeZone")) {
			...
			try {
				...
				pLog.error("Format of SafeZone wrong, must be decimal number: "
						+ getProperty("safeZone") + ", exception "
						+ e.getMessage());
				System.exit(1);
			}
		}
		...
		if (!containsKey("FILTER_MOMENTUM")) {
			...
			if (!getProperty("FILTER_MOMENTUM").equals("yes")
				...
				pLog.error("Format of momentum filter rule wrong ("
						+ getProperty("FILTER_MOMENTUM")
						+ "), must be either yes or no");
				System.exit(1);
			}
		}
		if (!containsKey("FILTER_ENTRY_BAR_HIGH")) {
			...
			if (!getProperty("FILTER_ENTRY_BAR_HIGH").equals("yes")
				...
				pLog.error("Format of entry bar high filter rule wrong ("
						+ getProperty("FILTER_ENTRY_BAR_HIGH")
						+ "), must be either yes or no");
				System.exit(1);
			}
		}
		if (!containsKey("FILTER_ENTRY_BAR_LOW")) {
			...
			if (!getProperty("FILTER_ENTRY_BAR_LOW").equals("yes")
				...
				pLog.error("Format of entry bar low filter rule wrong ("
						+ getProperty("FILTER_ENTRY_BAR_LOW")
						+ "), must be either yes or no");
				System.exit(1);
			}
		}
		if (!containsKey("trailProfit")) {
			...
			if (!getProperty("trailProfit").equals("yes")
				...
				pLog.error("Format of 4 hours timeframe filter wrong ("
						+ getProperty("trailProfit")
						+ "), must be either yes or no");
				System.exit(1);
			}
		}
		if (!containsKey("trailProfit")) {
			...
			if (!getProperty("trailProfit").equals("yes")
					&& !getProperty("trailProfit").equals("no")) {
				pLog.error("Format of traling profit protection rule wrong ("
						+ getProperty("trailProfit")
						+ "), must be either yes or no");
				System.exit(1);
			}
		...
		if (!containsKey("USE_FILTERS")) {
			...
			if (!getProperty("USE_FILTERS").equals("yes")
				...
				pLog.error("Format of filter usage rule wrong ("
						+ getProperty("USE_FILTERS")
						+ "), must be either yes or no");
				System.exit(1);
			}
		}
		if (!containsKey("REPORT_FILTERS")) {
			...
			if (!getProperty("REPORT_FILTERS").equals("yes")
				...
				pLog.error("Format of filter reports rule wrong ("
						+ getProperty("REPORT_FILTERS")
						+ "), must be either yes or no");
				System.exit(1);
			}
		}
