...
/**
 * Writes signals into DB instead of preparing HTML email elements Conditions:
 * 1. channel squeeze (BBands within KChannel) 2. but price closed above / below
 * upper / lower BBand
 * 
 * @author Sascha
 * 
 */
public class RangeExtremesStrategy extends BaseFlexElement implements
		IFlexEmailElement {
	...
	public String print(Instrument instrument, Period pPeriod, IBar bidBar,
		...
		String color = getWhite(), value = new String("Trading range extreme"), sqlStr = new String(
		...
		DecimalFormat df = instrument.getPipScale() == 2 ? FXUtils.df2
				: FXUtils.df5;
		if (isBullishRangeExtreme(mailStringsMap, mailValuesMap, instrument,
				pPeriod, history, indicators, vola, bidBar)) {
			value = instrument.toString()
					+ ": Bulish volatility drop breakout signal ! BUY STP: "
					+ df.format(high)
					+ ", ATR "
			...
			sqlStr += ", ValueName1, ValueD1) "
					+ SignalUtils.getSignalInsertValues(
							SignalUtils.RangeExtremeID, FXUtils
		...
		} else if (isBearishRangeExtreme(mailStringsMap, mailValuesMap,
				instrument, pPeriod, history, indicators, vola, bidBar)) {
			value = "Bearish volatility drop breakout signal ! SELL STP: "
					+ df.format(low)
					+ ", ATR "
					+ FXUtils.df1.format(atr)
					+ ", SL "
	...
	public SignalResult detectSignal(Instrument instrument, Period pPeriod,
		...
		if (isBullishRangeExtreme(mailStringsMap, mailValuesMap, instrument,
				pPeriod, history, indicators, vola, bidBar)) {
			value = instrument.toString()
					+ ": Bulish range extreme signal ! BUY STP: "
					+ df.format(high)
					+ ", ATR "
					+ FXUtils.df1.format(atr)
					+ ", SL "
			...
			result.insertSQL = sqlStr;
				...
				instrument, pPeriod, history, indicators, vola, bidBar)) {
			value = "Bearish range extreme signal ! SELL STP: "
					+ df.format(low)
					+ ", ATR "
					+ FXUtils.df1.format(atr)
					+ ", SL "
	...
	/**
	 * Signal criteria: - flat trend regime - not a volatility squeeze - bullish
	 * signal: bullish candles on the bottom of the channel - bearish signal:
	 * bearish candles on the top of the channel
	 */
	private boolean isBearishRangeExtreme(Map<String, String> mailStringsMap,
		...
		double squeeze = mailValuesMap.get(
				"volatility"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getDoubleValue();
		if (squeeze <= 70.0)
		...
		double flatRegime = mailValuesMap.get(
				"MAsDistance"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getDoubleValue();
		...
		if (candles == null || candles.length() == 0
				|| !candles.contains("BEARISH"))
			return false;

		double triggerHighChannelPos = mailValuesMap.get(
				"bearishCandleTriggerChannelPos"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getDoubleValue(), triggerHighKChannelPos = mailValuesMap.get(
				"bearishCandleTriggerKChannelPos"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getDoubleValue();
	...
	private boolean isBullishRangeExtreme(Map<String, String> mailStringsMap,
			Map<String, FlexLogEntry> mailValuesMap, Instrument instrument,
			Period pPeriod, IHistory history, IIndicators indicators,
		...
		String candles = mailStringsMap.get("CandleTrigger"
				+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()));
		if (candles == null || candles.length() == 0
				|| !candles.contains("BULLISH"))
			return false;

		double triggerLowChannelPos = mailValuesMap.get(
				"bullishCandleTriggerChannelPos"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getDoubleValue(), triggerLowKChannelPos = mailValuesMap.get(
				"bullishCandleTriggerKChannelPos"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getDoubleValue();
