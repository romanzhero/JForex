 ...
 * Condition: current bar close is highest / lowest in last N 
 * @author Sascha
 *
 */
public class DonchianBreakoutSlow extends BaseFlexElement implements IFlexEmailElement {
	
	protected boolean signalFound = false;
	protected final int PERIODS = 55;
	...
	public IFlexEmailElement cloneIt() {
		return new DonchianBreakoutSlow();
	}
	...
	public String print(Instrument instrument, Period pPeriod, IBar bidBar,
		...
		String
			color = getWhite(),
			value = new String("Donchian channel breakout"),
		...
		if (isBullishBreakout(mailStringsMap, mailValuesMap, instrument, pPeriod, history, indicators, vola, bidBar)) {
			value = instrument.toString() + ": Bulish Donchian channel breakout signal ! BUY STP: " + df.format(high) 
			...
			sqlStr += ", ValueName1, ValueD1) "
					+ SignalUtils.getSignalInsertValues(SignalUtils.DonchianBreakoutSlowID, 
			...
			try {
				ResultSet signalsDone = FXUtils.dbReadQuery(logDB, "SELECT count(*) as signal_exists FROM " 
						+ FXUtils.getDbToUse() + ".tsignal WHERE option_id = " + SignalUtils.DonchianBreakoutSlowID
			...
			value = "Bearish Donchian channel breakout signal ! SELL STP: " + df.format(low) 
					+ ", ATR " + FXUtils.df1.format(atr)
					+ ", SL " + df.format(FXUtils.roundToPip(high + atr / Math.pow(10, instrument.getPipScale()) * 1.5, instrument));
			color = getRed();
			signalFound = true;
	...
	private boolean isBearishBreakout(Map<String, String> mailStringsMap, Map<String, FlexLogEntry> mailValuesMap, Instrument instrument, Period pPeriod, IHistory history, IIndicators indicators, Volatility vola, IBar bidBar) throws JFException {
		double[][] donchian = indicators.donchian(instrument, pPeriod, OfferSide.BID, PERIODS, Filter.WEEKENDS, 2, bidBar.getTime(), 0);
		double
			currDonchianBottom = donchian[1][1],
	    	prevDonchianBottom = donchian[1][0];
	...
	private boolean isBullishBreakout(Map<String, String> mailStringsMap, Map<String, FlexLogEntry> mailValuesMap, Instrument instrument, Period pPeriod, IHistory history, IIndicators indicators, Volatility vola, IBar askBar) throws JFException {
		double[][] donchian = indicators.donchian(instrument, pPeriod, OfferSide.BID, PERIODS, Filter.WEEKENDS, 2, askBar.getTime(), 0);
		double
			currDonchianTop = donchian[0][1],
	    	prevDonchianTop = donchian[0][0];
