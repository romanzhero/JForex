...
public class BarStatElement extends BaseFlexElement implements IFlexEmailElement {
...
return new String();
 ...
 public String print(Instrument instrument, Period pPeriod, IBar bidBar, List<FlexLogEntry> logLine, Connection logDB) {
	 ResultSet elementData = dbGetElementData(instrument, pPeriod, bidBar, logDB, "barStat", true);
	 if (elementData != null) {
		try {
			return new String("bar range vs. average range: " + FXUtils.df1.format(elementData.getDouble("barStat")) + " StDev(s) from average");  	
		} catch (SQLException e) {
			   System.out.print("Log database problem in BarStatElement: " + e.getMessage());
	           System.exit(1);
		}  
	 }	 
	 return null;
