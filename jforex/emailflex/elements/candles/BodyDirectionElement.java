...
public class BodyDirectionElement extends BaseFlexElement implements IFlexEmailElement {
...
return new String();
 ...
 public String print(Instrument instrument, Period pPeriod, IBar bidBar, List<FlexLogEntry> logLine, Connection logDB) {
	 ResultSet elementData = dbGetElementData(instrument, pPeriod, bidBar, logDB, "upperHandlePerc, barBodyPerc, lowerHandlePerc, barOpen, barClose", true);
	 if (elementData != null) {
		try {
			return new String("Last bar candle stats: upper handle = "  
					+ FXUtils.df1.format(elementData.getDouble("upperHandlePerc")) + "% / " + 
					(elementData.getDouble("barClose") > elementData.getDouble("barOpen") ? "BULLISH" : "BEARISH") + " body = "  
					+ FXUtils.df1.format(elementData.getDouble("barBodyPerc")) + "% / lower handle = "			
					+ FXUtils.df1.format(elementData.getDouble("lowerHandlePerc")) + "%");
		} catch (SQLException e) {
			   System.out.print("Log database problem in BodyDirectionElement: " + e.getMessage());
	           System.exit(1);
		}  
	 }	 
	 return null;
