...
public String printHTML(Instrument instrument, Period pPeriod, IBar bidBar,	Connection logDB) {
	...
	String res = new String("<tr><td><span style=\"display:block;\">Volatility</span><span style=\"display:block;\">ATR</span></td>");
	float 
		currVola = dbGetFloat(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "volatility", false),
		currATR = dbGetFloat(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "ATR", false);
	res += printCell(currVola, currATR);
	
	currVola = dbGetFloat(instrument, Period.FOUR_HOURS, bidBar, logDB, "volatility", false);
	currATR = dbGetFloat(instrument, Period.FOUR_HOURS, bidBar, logDB, "ATR", false);
	res += printCell(currVola, currATR);
	
	currVola = dbGetFloat(instrument, Period.THIRTY_MINS, bidBar, logDB, "volatility", true);
	currATR = dbGetFloat(instrument, Period.THIRTY_MINS, bidBar, logDB, "ATR", true);
	res += printCell(currVola, currATR);
...
private String printCell(float currVola, float currATR) {
	...
	if (currVola > 190.0)
		res += "<span style=\"background-color:" + getDarkRed() + "; display:block; margin:0 1px;\">Very high ! (" + FXUtils.df1.format(currVola) + "%)</span>";
	if (currVola > 150.0 && currVola <= 190.0)
		res += "<span style=\"background-color:" + getRed() + "; display:block; margin:0 1px;\">High ! (" + FXUtils.df1.format(currVola) + "%)</span>";
	else if (currVola < 70.0 && currVola >= 50)
		res += "<span style=\"background-color:" + getLightBlue() + "; display:block; margin:0 1px;\">Low ! (" + FXUtils.df1.format(currVola) + "%)</span>";
	else if (currVola < 50.0)
		res += "<span style=\"background-color:" + getDarkBlue() + "; display:block; margin:0 1px;\">Low ! (" + FXUtils.df1.format(currVola) + "%)</span>";
	else
		res += "<span style=\"background-color:" + getWhite() + "; display:block; margin:0 1px;\">" + FXUtils.df1.format(currVola) + "%</span>";

	res += "<span style=\"background-color:" + getWhite() + "; display:block; margin:0 1px;\">" + FXUtils.df1.format(currATR) + " pips</span>";
