...
public class TrendOverviewElement extends AbstractOverview {
	...
	public String printHTML(Instrument instrument, Period pPeriod, IBar bidBar,	Connection logDB) {
		...
		String trendID = dbGetString(instrument, pPeriod, bidBar, logDB, "TrendId", true);
		float trendStrength = dbGetFloat(instrument, pPeriod, bidBar, logDB, "MAsDistance", true);
		boolean twoRowsNeeded = false;
		
		String trendID4h = dbGetString(instrument, Period.FOUR_HOURS, bidBar, logDB, "TrendId", false);
		float trendStrength4h = dbGetFloat(instrument, Period.FOUR_HOURS, bidBar, logDB, "MAsDistance", false);
	
		String trendID1d = dbGetString(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "TrendId", false);
		float trendStrength1d = dbGetFloat(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "MAsDistance", false);
		...
		return "<tr style=\"background-color:#f4f4f4\"><td>Trend</td>"
				+ getHTMLtd(trendID1d, trendStrength1d, twoRowsNeeded && trendStrength1d >= -0.7)
				+ getHTMLtd(trendID4h, trendStrength4h, twoRowsNeeded && trendStrength4h >= -0.7)
				+ getHTMLtd(trendID, trendStrength, twoRowsNeeded && trendStrength >= -0.7)
				+ "</tr>";
	}
	
	
	private String getHTMLtd(String trendID, float trendStrength, boolean twoRows) {
		String 
			text = null,
			color = null,
			height = twoRows ? new String(" height:40px; line-height:40px;") : new String();
		if (trendStrength < -0.7) {
			text = new String(FXUtils.beautify("FLAT") + " (" + FXUtils.df1.format(trendStrength) + ")<br />(" + FXUtils.beautify(trendID) + ")");
			if (trendStrength < -1.0)
				color = getDarkBlue();
			else 
				color = getLightBlue();
		} else {
			text = new String(FXUtils.beautify(trendID) + " (" + FXUtils.df1.format(trendStrength) + ")");
		...
		return new String("<td><span style=\"background-color:"  
				+ color + "; display:block; margin:0 3px;" + height + "\">"
				+ text + "</span></td>");
	}
}