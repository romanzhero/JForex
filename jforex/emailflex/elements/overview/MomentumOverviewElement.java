...
public String printHTML(Instrument instrument, Period pPeriod, IBar bidBar, Connection logDB) {
	...
	String	
		macdCross1d = dbGetString(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "MACDCross", false),
		macdCross4h = dbGetString(instrument, Period.FOUR_HOURS, bidBar, logDB, "MACDCross", false),
		macdCross30min = dbGetString(instrument, pPeriod, bidBar, logDB, "MACDCross", true),
		res = new String();
	boolean atLeastOneMACDCross = macdCross1d.equals("BULL_CROSS_BELOW_ZERO") || macdCross1d.equals("BEAR_CROSS_ABOVE_ZERO")
			|| macdCross4h.equals("BULL_CROSS_BELOW_ZERO") || macdCross4h.equals("BEAR_CROSS_ABOVE_ZERO")
			|| macdCross30min.equals("BULL_CROSS_BELOW_ZERO") || macdCross30min.equals("BEAR_CROSS_ABOVE_ZERO");
	...
	String 
		currMACD = dbGetString(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "MACDState", false),
		currMACD_H = dbGetString(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "MACDHState", false),
		currStochState = dbGetString(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "StochState", false),
		currStochCrossState = dbGetString(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "StochsCross", false),
		currCountTxt = momentumCount(instrument, bidBar, logDB, Period.DAILY_SUNDAY_IN_MONDAY, false),
		currCountColor = getCountColor(currCountTxt, currMACD);
	float currStochDiff = dbGetFloat(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "StochFast", false)
		- dbGetFloat(instrument, Period.DAILY_SUNDAY_IN_MONDAY, bidBar, logDB, "StochSlow", false);
	
	res += "<td><span style=\"background-color:" + currCountColor + "; display:block; margin:0 1px;\">" + FXUtils.beautify(currCountTxt) + "</span>"
		+ printMACD(currMACD, macdCross1d, atLeastOneMACDCross)
		+ printMACD_H(currMACD_H)
		+ printStochState(currStochState)
		+ printStochsDiff(currStochDiff, currStochCrossState)
		+ "</td>";

	currMACD = dbGetString(instrument, Period.FOUR_HOURS, bidBar, logDB, "MACDState", false);
	currMACD_H = dbGetString(instrument, Period.FOUR_HOURS, bidBar, logDB, "MACDHState", false);
	currStochState = dbGetString(instrument, Period.FOUR_HOURS, bidBar, logDB, "StochState", false);
	currStochCrossState = dbGetString(instrument, Period.FOUR_HOURS, bidBar, logDB, "StochsCross", false);
	currCountTxt = momentumCount(instrument, bidBar, logDB, Period.FOUR_HOURS, false);
	currCountColor = getCountColor(currCountTxt, currMACD);
	currStochDiff = dbGetFloat(instrument, Period.FOUR_HOURS, bidBar, logDB, "StochFast", false)
		- dbGetFloat(instrument, Period.FOUR_HOURS, bidBar, logDB, "StochSlow", false);
	res += "<td><span style=\"background-color:" + currCountColor + "; display:block; margin:0 1px;\">" + FXUtils.beautify(currCountTxt) + "</span>"
	...
	currMACD = dbGetString(instrument, Period.THIRTY_MINS, bidBar, logDB, "MACDState", true);
	currMACD_H = dbGetString(instrument, Period.THIRTY_MINS, bidBar, logDB, "MACDHState", true);
	currStochState = dbGetString(instrument, Period.THIRTY_MINS, bidBar, logDB, "StochState", true);
	currStochCrossState = dbGetString(instrument, Period.THIRTY_MINS, bidBar, logDB, "StochsCross", true);
	currCountTxt = momentumCount(instrument, bidBar, logDB, Period.THIRTY_MINS, true);
	currCountColor = getCountColor(currCountTxt, currMACD);
	currStochDiff = dbGetFloat(instrument, Period.THIRTY_MINS, bidBar, logDB, "StochFast", true)
		- dbGetFloat(instrument, Period.THIRTY_MINS, bidBar, logDB, "StochSlow", true);
	res += "<td><span style=\"background-color:" + currCountColor + "; display:block; margin:0 1px;\">" + FXUtils.beautify(currCountTxt) + "</span>"
...
private String printStochsDiff(float currStochDiff, String currStochCrossState) {
	String 
		text = null,
		color = null;
	if (currStochCrossState != null && !currStochCrossState.equals("NONE")) {
		text = new String(FXUtils.beautify(currStochCrossState) + ", " + FXUtils.df1.format(currStochDiff));
		if (currStochCrossState.toUpperCase().equals("BULLISH_CROSS_FROM_OVERSOLD"))
			color = new String("#090");
		else if (currStochCrossState.toUpperCase().equals("BULLISH_CROSS"))
			color = new String("#0C3");
		else if (currStochCrossState.toUpperCase().equals("BEARISH_CROSS_FROM_OVERBOUGTH"))
	...
	return new String("<span style=\"background-color:" + color + "; display:block; margin:0 1px;\">" + text + "</span>");
}
...
private String printStochState(String currStochState) {
	...
	if (currStochState.toUpperCase().equals("OVERSOLD_SLOW"))
		color = new String("#0F6");
	else if (currStochState.toUpperCase().equals("RAISING_IN_MIDDLE") || currStochState.toUpperCase().equals("OVERBOUGHT_FAST"))
		color = new String("#0C3");
	else if (currStochState.toUpperCase().equals("OVERBOUGHT_BOTH"))
		color = new String("#090");
	else if (currStochState.toUpperCase().equals("OVERBOUGHT_SLOW"))
		color = new String("#F66");
	else if (currStochState.toUpperCase().equals("FALLING_IN_MIDDLE") || currStochState.toUpperCase().equals("OVERSOLD_FAST"))
		color = new String("#F00");
	...
	return new String("<span style=\"background-color:" + color + "; display:block; margin:0 1px;\">" + FXUtils.beautify(currStochState) + "</span>");
}
...
private String printMACD_H(String currMACD_H) {
	...
	return new String("<span style=\"background-color:" + color + "; display:block; margin:0 1px;\">" + FXUtils.beautify(currMACD_H) + "</span>");
}


private String printMACD(String currMACD, String macdCross, boolean atLeastOneMACDCross) {
	String 
		res = new String(),
		color = null;
	...
	res += "<span style=\"background-color:" + color + "; display:block; margin:0 1px;\">" + FXUtils.beautify(currMACD) + "</span>";		
	if (atLeastOneMACDCross) {
		if (macdCross.contains("CROSS")) {
			if (macdCross.toLowerCase().equals("bull_cross_below_zero"))
				color = new String("#0F6");
			else if (macdCross.toLowerCase().equals("bull_cross_above_zero"))
				color = new String("#090");
			else if (macdCross.toLowerCase().equals("bear_cross_above_zero"))
				color = new String("#F66");
			else if (macdCross.toLowerCase().equals("bear_cross_below_zero"))
				color = new String("#C00");
			res += "<span style=\"background-color:" + color + "; display:block; margin:0 1px;\">" + FXUtils.beautify(macdCross) + "</span>";					
		}
		else 
			res += "<span style=\"background-color:" + color + "; display:block; margin:0 1px;\">&nbsp;</span>";					
	}
...
private String momentumCount(Instrument instrument, IBar bidBar, Connection logDB, Period timeFrameID, boolean exact) {
	int 
		upCount = 0,
		downCount = 0;
	String currMomentum = dbGetString(instrument, timeFrameID, bidBar, logDB, "MACDState", exact);
	...
	currMomentum = dbGetString(instrument, timeFrameID, bidBar, logDB, "MACDHState", exact);
	if (currMomentum != null && currMomentum.length() > 0) {
		if (currMomentum.contains("RAISING") || currMomentum.contains("TICKED_UP"))
			upCount++;
		else
	...
	double 
		StochFast = dbGetFloat(instrument, timeFrameID, bidBar, logDB, "StochFast", exact),
		StochSlow = dbGetFloat(instrument, timeFrameID, bidBar, logDB, "StochSlow", exact);
	...
	String 
		MACDState = dbGetString(instrument, timeFrameID, bidBar, logDB, "MACDState", exact),
		suffix = null;
