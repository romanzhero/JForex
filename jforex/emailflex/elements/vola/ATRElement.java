...
return new String();
 ...
 public String print(Instrument instrument, Period pPeriod, IBar bidBar, List<FlexLogEntry> logLine, Connection logDB) {
		...
		String 
			value = new String("ATR: " + mailStringsMap.get("ATR" + FXUtils.timeFrameNamesMap.get(pPeriod.toString()))),
			valueHigh = new String("bar high: " + mailStringsMap.get("barHigh" + FXUtils.timeFrameNamesMap.get(pPeriod.toString()))),
			valueLow = new String("bar low: " + mailStringsMap.get("barLow" + FXUtils.timeFrameNamesMap.get(pPeriod.toString()))),
			color = getWhite();		
		
		return new String("<tr><td><span style=\"background-color:" + color + "; display:block; margin:0 1px; \">" 
				+ value + "; " + valueHigh + " / " + valueLow + "</span></td></tr>"); 
 }
