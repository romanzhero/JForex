 ...
 * 
 */
public class SqueezeBreakout extends BaseFlexElement implements
		IFlexEmailElement {
	...
	public String print(Instrument instrument, Period pPeriod, IBar bidBar,
		...
		String color = getWhite(), value = new String(
				"Volatility squeeze breakout"), sqlStr = new String(
		...
		if (isBullishBreakout(mailStringsMap, mailValuesMap, instrument,
			...
			sqlStr += ", ValueName1, ValueD1) "
					+ SignalUtils.getSignalInsertValues(
							SignalUtils.SqueezeBreakoutID, FXUtils
	...
	private boolean isBearishBreakout(Map<String, String> mailStringsMap,
		...
		double[][] bBands = indicators.bbands(instrument, pPeriod,
				OfferSide.BID, AppliedPrice.CLOSE, 20, 2.0, 2.0, MaType.SMA,
				Filter.WEEKENDS, 2, bidBar.getTime(), 0);
		double currBBandsTop = bBands[Channel.TOP][1], currBBandsMiddle = bBands[Channel.MIDDLE][1], currBBandsBottom = bBands[Channel.BOTTOM][1], prevBBandsBottom = bBands[Channel.BOTTOM][0], twoStDev = currBBandsTop
				- currBBandsMiddle, twoATRs = 2 * vola.getATR(instrument,
				pPeriod, OfferSide.BID, bidBar.getTime(), 20), squeeze = twoStDev
				/ twoATRs * 100.0;
		if (squeeze > 70.0)
			return false;
	...
	private boolean isBullishBreakout(Map<String, String> mailStringsMap,
		...
		double currBBandsTop = bBands[Channel.TOP][1], currBBandsMiddle = bBands[Channel.MIDDLE][1], prevBBandsTop = bBands[Channel.TOP][0], twoStDev = currBBandsTop
				- currBBandsMiddle, twoATRs = 2 * vola.getATR(instrument,
				pPeriod, OfferSide.BID, askBar.getTime(), 20), squeeze = twoStDev
				/ twoATRs * 100.0;
		if (squeeze > 70.0)
			return false;
		// need previous bar too...
