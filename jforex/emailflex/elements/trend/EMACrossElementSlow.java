 ...
 * If there is cross betweem simple MA20 and MA50 it calculates all necessary entry data 
 * and writes it into tsignal table
 * @author Sascha
 *
 */
public class EMACrossElementSlow extends BaseFlexElement implements IFlexEmailElement {
	...
	public IFlexEmailElement cloneIt() {
		return new EMACrossElementSlow();
	}
	...
	public String print(Instrument instrument, Period pPeriod, IBar bidBar,
		...
		if (isBullishEMACross(mailStringsMap, mailValuesMap, instrument, pPeriod, history, indicators, bidBar)) {
			value = instrument.toString() + ": Bulish EMA cross (EMA20 / EMA50) trend following signal ! BUY STP: " + df.format(high) 
			...
			sqlStr += ", ValueName1, ValueD1) "
					+ SignalUtils.getSignalInsertValues(SignalUtils.EMACrossSlowID, 
			...
			try {
				ResultSet signalsDone = FXUtils.dbReadQuery(logDB, "SELECT count(*) as signal_exists FROM " 
						+ FXUtils.getDbToUse() + ".tsignal WHERE option_id = " + SignalUtils.EMACrossSlowID
		...
		else if (isBearishEMACross(mailStringsMap, mailValuesMap, instrument, pPeriod, history, indicators, bidBar)) {
			value = "Bearish EMA cross (EMA20 / EMA50) trend following signal ! SELL STP: " + df.format(low) 
					+ ", ATR " + FXUtils.df1.format(atr)
					+ ", SL " + df.format(FXUtils.roundToPip(high + atr / Math.pow(10, instrument.getPipScale()) * 1.5, instrument));
			color = getRed();
			signalFound = true;
	...
	private boolean isBearishEMACross(Map<String, String> mailStringsMap, Map<String, FlexLogEntry> mailValuesMap, Instrument instrument, Period pPeriod, IHistory history, IIndicators indicators, IBar bidBar) throws JFException {
		double[] 
				ema50 = indicators.ema(instrument, pPeriod, OfferSide.ASK, AppliedPrice.CLOSE, 50, Filter.WEEKENDS, 2, bidBar.getTime(), 0),
				ema100 = indicators.ema(instrument, pPeriod, OfferSide.ASK, AppliedPrice.CLOSE, 100, Filter.WEEKENDS, 2, bidBar.getTime(), 0);
		double
			prevSMA50 = ema50[0], currSMA50 = ema50[1],
			prevSMA100 = ema100[0], currSMA100 = ema100[1];
	
		return prevSMA50 >= prevSMA100 && currSMA50 < currSMA100;
	}

	private boolean isBullishEMACross(Map<String, String> mailStringsMap, Map<String, FlexLogEntry> mailValuesMap, Instrument instrument, Period pPeriod, IHistory history, IIndicators indicators, IBar askBar) throws JFException {
		double[] 
				ema50 = indicators.ema(instrument, pPeriod, OfferSide.ASK, AppliedPrice.CLOSE, 50, Filter.WEEKENDS, 2, askBar.getTime(), 0),
				ema100 = indicators.ema(instrument, pPeriod, OfferSide.ASK, AppliedPrice.CLOSE, 100, Filter.WEEKENDS, 2, askBar.getTime(), 0);
		double
			prevSMA50 = ema50[0], currSMA50 = ema50[1],
			prevSMA100 = ema100[0], currSMA100 = ema100[1];
