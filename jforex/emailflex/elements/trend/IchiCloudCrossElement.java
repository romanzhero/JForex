...
public class IchiCloudCrossElement extends BaseFlexElement implements IFlexEmailElement {
	...
	public String print(Instrument instrument, Period pPeriod, IBar bidBar, List<FlexLogEntry> logLine, Connection logDB) {
		...
		FlexLogEntry logEntry = mailValuesMap.get("IchiCloudDist" + FXUtils.timeFrameNamesMap.get(pPeriod.toString()));
		...
		if (valueDbl > 2.0) {
			color = getRed();
			value = "Extreme distance from Ichimoku cloud (" + logEntry.getFormattedValue() + ") !";
			signalFound = true;
		}
