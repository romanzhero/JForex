 ...
 * If there is Ichi cloud breakout it calculates all necessary entry data and
 * writes it into " + FXUtils.getDbToUse() + ".tsignal table
 * 
 * @author Sascha
 * 
 */
public class IchiCloudBreakoutElement extends BaseFlexElement implements
		IFlexEmailElement {
	...
	public String print(Instrument instrument, Period pPeriod, IBar bidBar,
		...
		String color = getWhite(), value = mailStringsMap.get("IchiCloudCross"
		...
		if (value.equals(Trend.ICHI_CLOUD_CROSS.BULLISH.toString())) {
			value = instrument.toString()
					+ ": Bullish breakout from Ichimoku cloud (signal) ! BUY STP: "
					+ df.format(high)
					+ ", ATR "
			...
			sqlStr += ", ValueName1, ValueD1) "
					+ SignalUtils.getSignalInsertValues(
							SignalUtils.IchiCloudBreakoutID, FXUtils
							...
							(high + low) / 2, FXUtils.roundToPip(high - atr
									...
									* 1.5, instrument), Double
									.parseDouble(FXUtils.df1.format(atr)));
			...
			try {
				ResultSet signalsDone = FXUtils.dbReadQuery(
						...
						"SELECT count(*) as signal_exists FROM "
								...
								+ SignalUtils.IchiCloudBreakoutID
								+ " AND instrument_id = "
			...
			value = "Bearish breakout from Ichimoku cloud (signal) ! SELL STP: "
					+ df.format(low)
					+ ", ATR "
					+ FXUtils.df1.format(atr)
					+ ", SL "
