 ...
 * simple trend following signal of buy dip / sell rallies it calculates all
 * necessary entry data and writes it into tsignal table
 * 
 * @author Sascha
 * 
 */
public class BuyDipsSellRalliesElement extends BaseFlexElement implements
		IFlexEmailElement {
	...
	public String print(Instrument instrument, Period pPeriod, IBar bidBar,
		...
		String color = getWhite(), value = mailStringsMap
				.get("BuyDipsSellRallies"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString())), sqlStr = new String(
		...
		if (isUptrendDip(mailStringsMap, mailValuesMap, pPeriod)) {
			value = instrument.toString()
					+ ": Buy dip bullish trend following signal ! BUY STP: "
					+ df.format(high)
					+ ", ATR "
			...
			sqlStr += ", ValueName1, ValueD1) "
					+ SignalUtils.getSignalInsertValues(
							SignalUtils.BuyDipsSellRalliesID, FXUtils
			...
			try {
				ResultSet signalsDone = FXUtils.dbReadQuery(
						...
						"SELECT count(*) as signal_exists FROM "
								...
								+ SignalUtils.BuyDipsSellRalliesID
								+ " AND instrument_id = "
			...
			value = "Sell rally bearish trend following signal ! SELL STP: "
					+ df.format(low)
					+ ", ATR "
					+ FXUtils.df1.format(atr)
					+ ", SL "
	...
	private boolean isDownTrendRally(Map<String, String> mailStringsMap,
			Map<String, FlexLogEntry> mailValuesMap, Period pPeriod) {
		String trendState = mailStringsMap.get("TrendId"
				+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()));
		double trendStrength = ((Double) mailValuesMap.get(
				"MAsDistance"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getValue()).doubleValue();
		if (!((trendState.equals("DOWN_STRONG") || trendState
				.equals("DOWN_MILD")) && trendStrength > -0.7))
			return false;
		String candleTrigger = isBearishCandle(mailStringsMap, mailValuesMap,
				pPeriod);
		if (candleTrigger == null || candleTrigger.contains("3"))
			return false;

		Double triggerHighChannelPos = (Double) mailValuesMap.get(
				"bearishCandleTriggerChannelPos"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getValue(), volatility = (Double) mailValuesMap.get(
				"volatility"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getValue(), triggerHighKChannelPos = (Double) mailValuesMap
				.get("bearishCandleTriggerKChannelPos"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getValue();
	...
	private boolean isUptrendDip(Map<String, String> mailStringsMap,
			Map<String, FlexLogEntry> mailValuesMap, Period pPeriod) {
		String trendState = mailStringsMap.get("TrendId"
				+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()));
		double trendStrength = ((Double) mailValuesMap.get(
		...
		if (!((trendState.equals("UP_STRONG") || trendState.equals("UP_MILD")) && trendStrength > -0.7))
			return false;
		String candleTrigger = isBullishCandle(mailStringsMap, mailValuesMap,
				pPeriod);
		if (candleTrigger == null || candleTrigger.contains("3"))
			return false;

		Double triggerLowChannelPos = (Double) mailValuesMap.get(
				"bullishCandleTriggerChannelPos"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getValue(), volatility = (Double) mailValuesMap.get(
				"volatility"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getValue(), triggerLowKChannelPos = (Double) mailValuesMap
				.get("bullishCandleTriggerKChannelPos"
						+ FXUtils.timeFrameNamesMap.get(pPeriod.toString()))
				.getValue();
	...
	private String isBullishCandle(Map<String, String> mailStringsMap,
			Map<String, FlexLogEntry> mailValuesMap, Period pPeriod) {
		String candles = mailStringsMap.get("CandleTrigger"
				+ FXUtils.timeFrameNamesMap.get(pPeriod.toString())), bullishCandles = null;
	...
	private String isBearishCandle(Map<String, String> mailStringsMap,
			Map<String, FlexLogEntry> mailValuesMap, Period pPeriod) {
		String candles = mailStringsMap.get("CandleTrigger"
				+ FXUtils.timeFrameNamesMap.get(pPeriod.toString())), bearishCandles = null;
		if (candles != null && !candles.toUpperCase().equals("NONE")) {
			int positionOfAnd = candles.indexOf(" AND ");
			if (positionOfAnd > 0) {
				String firstCandle = candles.substring(0, positionOfAnd);
