 ...
 *
 */
public class SMACrossElementFast extends BaseFlexElement implements IFlexEmailElement {
	...
	public IFlexEmailElement cloneIt() {
		return new SMACrossElementFast();
	}
	...
	public String print(Instrument instrument, Period pPeriod, IBar bidBar,
		...
		if (isBullishSMACross(mailStringsMap, mailValuesMap, instrument, pPeriod, history, indicators, bidBar)) {
			value = instrument.toString() + ": Bulish SMA cross (MA20 / MA50) trend following signal ! BUY STP: " + df.format(high) 
			...
			sqlStr += ", ValueName1, ValueD1) "
					+ SignalUtils.getSignalInsertValues(SignalUtils.SMACrossFastID, 
			...
			try {
				ResultSet signalsDone = FXUtils.dbReadQuery(logDB, "SELECT count(*) as signal_exists FROM " 
						+ FXUtils.getDbToUse() + ".tsignal WHERE option_id = " + SignalUtils.SMACrossFastID
		...
		else if (isBearishSMACross(mailStringsMap, mailValuesMap, instrument, pPeriod, history, indicators, bidBar)) {
			value = "Bearish SMA cross (MA20 / MA50) trend following signal ! SELL STP: " + df.format(low) 
					+ ", ATR " + FXUtils.df1.format(atr)
					+ ", SL " + df.format(FXUtils.roundToPip(high + atr / Math.pow(10, instrument.getPipScale()) * 1.5, instrument));
			color = getRed();
			signalFound = true;
	...
	private boolean isBearishSMACross(Map<String, String> mailStringsMap, Map<String, FlexLogEntry> mailValuesMap, Instrument instrument, Period pPeriod, IHistory history, IIndicators indicators, IBar bidBar) throws JFException {
		double[] 
				sma20 = indicators.sma(instrument, pPeriod, OfferSide.ASK, AppliedPrice.CLOSE, 20, Filter.WEEKENDS, 2, bidBar.getTime(), 0),
				sma50 = indicators.sma(instrument, pPeriod, OfferSide.ASK, AppliedPrice.CLOSE, 50, Filter.WEEKENDS, 2, bidBar.getTime(), 0);
		double
			prevSMA20 = sma20[0], currSMA20 = sma20[1],
			prevSMA50 = sma50[0], currSMA50 = sma50[1];
	
		return prevSMA20 >= prevSMA50 && currSMA20 < currSMA50;
	}

	private boolean isBullishSMACross(Map<String, String> mailStringsMap, Map<String, FlexLogEntry> mailValuesMap, Instrument instrument, Period pPeriod, IHistory history, IIndicators indicators, IBar askBar) throws JFException {
		double[] 
				sma20 = indicators.sma(instrument, pPeriod, OfferSide.ASK, AppliedPrice.CLOSE, 20, Filter.WEEKENDS, 2, askBar.getTime(), 0),
				sma50 = indicators.sma(instrument, pPeriod, OfferSide.ASK, AppliedPrice.CLOSE, 50, Filter.WEEKENDS, 2, askBar.getTime(), 0);
		double
			prevSMA20 = sma20[0], currSMA20 = sma20[1],
			prevSMA50 = sma50[0], currSMA50 = sma50[1];
