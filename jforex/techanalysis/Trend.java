...
public class Trend {
	...
	public enum TREND_STATE {
		...
		DOWN_MILD,
		DOWN_STRONG,
		NONE // shouldn't happen, for debugging
	}
	...
	public class IchiDesc {
		public double 
			...
			widthRel;
		public boolean
			isBullishSlowLineCross = false,
			isBearishSlowLineCross = false,
			
			// needed for cases when lines turn adverse (to trade) AFTER cross of slow line
			isCloseAboveSlowLine = false,
			
			isBullishCloudCross = false,
			isBearishCloudCross = false,
		...
		public String 
			slowLineState,
			fastLineState,
			bottomBorderDirection,
			topBorderDirection;
		
		public String linesState() {
			if (slowLineState.equals("FLAT") && fastLineState.equals("FLAT"))
				return "FLAT";
			else if ((slowLineState.equals("FALLING") || slowLineState.equals("TICKED_DOWN") || slowLineState.equals("FLAT"))
					&& !fastLineState.equals("RAISING") && !fastLineState.equals("TICKED_UP"))
				return "BEARISH";
			else if ((slowLineState.equals("RAISING") || slowLineState.equals("TICKED_UP") || slowLineState.equals("FLAT"))
					&& !fastLineState.equals("FALLING") && !fastLineState.equals("TICKED_DOWN"))
				return "BULLISH";
			else
	...
	// current trend definition with Up = MA50 > MA100 && MA20 > MA100 and Down = MA50 < MA100 && MA20 < MA100
	// probably too simplistic, especially in 4h uptrend clearly also when MA100 > MA50; 
	// TODO: better study role of MA200, it's position versus all others... 
    public boolean isStrongUpTrendBy3MAs(Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time, int lookback, double stDevFilter) throws JFException
    {
        ...
        {
        	// check that MAs difference is WELL within first stdev i.e. greater then mean - 0.7 stdev 
        	// (subject to stat analysis)
        	return getUptrendMAsMaxDifStDevPos(instrument, pPeriod, side, appliedPrice, time, lookback) > stDevFilter;
        }
        else 
        {
        	return false;
    ...
    /**
     * @return
     * 1 - strong downtrend, MAs in descending order from slowest to fastest
     * 2 - weak downtrend, top-bottom order of MAs MA100, MA20, MA50, 
     * 3 - downtrend / flat, top-bottom order of MAs MA20, MA100, MA50  
     * 4 - uptrend / flat, top-bottom order of MAs MA50, MA100, MA20
     * 5 - weak uptrend, top-bottom order of MAs MA50, MA20, MA100
     * 6 - strong uptrend, top-bottom order of MAs MA20, MA50, MA100
     * @throws JFException
     */
    public int getTrendId(Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time) throws JFException
    {
        double MA20 = indicators.sma(instrument, pPeriod, side, appliedPrice, 20, 
        		Filter.WEEKENDS, 1, time, 0)[0];
        double MA50 = indicators.sma(instrument, pPeriod, side, appliedPrice, 50, 
     ...
     * @throws JFException
     */
    public TREND_STATE getTrendState(Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time) throws JFException
    {
        double MA20 = indicators.sma(instrument, pPeriod, side, appliedPrice, 20, 
        		Filter.WEEKENDS, 1, time, 0)[0];
        double MA50 = indicators.sma(instrument, pPeriod, side, appliedPrice, 50, 
        ...
        double MA100 = indicators.sma(instrument, pPeriod, side, appliedPrice, 100, 
        		Filter.WEEKENDS, 1, time, 0)[0];
//        double MA20 = indicators.sma(instrument, pPeriod, side, appliedPrice, 20, 0);
//        double MA50 = indicators.sma(instrument, pPeriod, side, appliedPrice, 50, 0);
//        double MA100 = indicators.sma(instrument, pPeriod, side, appliedPrice, 100, 0);
    ...
    public boolean isMA200Lowest(Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time) throws JFException
    {
        ...
        double MA100 = indicators.sma(instrument, pPeriod, side, appliedPrice, 100, 
        		Filter.WEEKENDS, 1, time, 0)[0];
        double MA200 = indicators.sma(instrument, pPeriod, side, appliedPrice, 200, 
        		Filter.WEEKENDS, 1, time, 0)[0];
     ...
     * 2. iterate over all three arrays at the same time
     * 3. for each index value:
     * 		3.1 check if MAs in uptrend position. If not skip loop
     * 		3.2 find greatest difference between values
     * 		3.3 put it into result list 
     * 4. convert list to array (less elements then lookback period !)
     * 5. calculate stdev using result array as input
     * 
     * First return element is mean and 2nd stdev
     * MAs should be put in a queue so for each bar only ONE more set of values are fetched,
     * not each time all 1'000 ! Just first time at start ! 
     */
    ...
    public double getUptrendMAsMaxDifStDevPos(Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time, int lookback) throws JFException 
    {
        ...
        if (MA50 > MA100 && MA20 > MA100)
        {
        	double[] stDevStats = uptrendMAsDifferenceStDevStats(instrument, pPeriod, side, appliedPrice, time, lookback);
    ...
    public boolean isStrongDownTrendBy3MAs(Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time, int lookback, double stDevFilter) throws JFException
    {
        double MA20 = indicators.sma(instrument, pPeriod, side, appliedPrice, 20, 
        		Filter.WEEKENDS, 1, time, 0)[0];
        double MA50 = indicators.sma(instrument, pPeriod, side, appliedPrice, 50, 
		...
		if (MA50 < MA100 && MA20 < MA100)
        {
        	return getDowntrendMAsMaxDifStDevPos(instrument, pPeriod, side, appliedPrice, time, lookback) > stDevFilter;
        }
        else 
        {
        	return false;
	...
	protected double[] getRawDowntrendMAsDifferences(Instrument instrument,	Period pPeriod, OfferSide side,	IIndicators.AppliedPrice appliedPrice, long time, int lookBack)	throws JFException {
        ...
        for (int i = 0; i < MAs20.length; i++)
        {
        	if (MAs50[i] < MAs100[i] && MAs20[i] < MAs100[i])
        	{
				downTrendValues.add(new Double(maxMAsDifference(MAs20[i], MAs50[i], MAs100[i])));
        	}
        }
    ...
    public double getDowntrendMAsMaxDifStDevPos(Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time, int lookback) throws JFException 
    {
        ...
        if (MA50 < MA100 && MA20 < MA100)
        {
        	double[] stDevStats = downtrendMAsDifferenceStDevStats(instrument, pPeriod, side, appliedPrice, time, lookback);
        	return (maxMAsDifference(MA20, MA50, MA100) - stDevStats[0]) / stDevStats[1];
        }
        else 
        {
    ...
    /**
     * calculates mean and StDev for ALL max MAs difference over the lookback period regardless of the trend  
     * @return 
     */
    ...
    public double[] getADXs(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
    	...
    	result[0] = indicators.adx(instrument, pPeriod, side, 14, Filter.WEEKENDS, 1, time, 0)[0];
    	result[1] = indicators.plusDi(instrument, pPeriod, side, 14, Filter.WEEKENDS, 1, time, 0)[0];
    	result[2] = indicators.minusDi(instrument, pPeriod, side, 14, Filter.WEEKENDS, 1, time, 0)[0];
	 ...
	 * @return 
	 */
	public double getAbsMAsDifference(Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time) throws JFException
	{
        double MA20 = indicators.sma(instrument, pPeriod, side, appliedPrice, 20, 
        		Filter.WEEKENDS, 1, time, 0)[0];
        double MA50 = indicators.sma(instrument, pPeriod, side, appliedPrice, 50, 
		...
		return maxMAsDifference(MA20, MA50, MA100) * Math.pow(10, instrument.getPipScale());
	}
    
	public ICHI_CLOUD_CROSS isIchiCloudCross(IHistory history, Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time) throws JFException
    {
	    double[][] 
	             // used for cloud borders, therefore moved kijun periods in past ! Cloud is drawn in future !
	             i_sh = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, 1 + kijun, time, 0),
	             i_sh_1 = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, 2 + kijun, time, 0);
	         ...
	         double
	             ...
	             i_cloudTop_1 = Math.round(Math.max(i_sh_1[SENOKU_A][0], i_sh_1[SENOKU_B][0]) * Math.pow(10, instrument.getPipScale())),
	             i_cloudBottom_1 = Math.round(Math.min(i_sh_1[SENOKU_A][0], i_sh_1[SENOKU_B][0]) * Math.pow(10, instrument.getPipScale()));
	             
	         double
	             cloudTop = i_cloudTop / Math.pow(10, instrument.getPipScale()),
	             cloudBottom = i_cloudBottom / Math.pow(10, instrument.getPipScale()),
	             cloudTop_1 = i_cloudTop_1 / Math.pow(10, instrument.getPipScale()),
	             cloudBottom_1 = i_cloudBottom_1 / Math.pow(10, instrument.getPipScale());
	             
	         List<IBar> last2bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, 2, time, 0);
	...
	public boolean isIchiLongCancel(IHistory history, Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time) throws JFException
    {
	    double[][] 
	             // used for cloud borders, therefore moved kijun periods in past ! Cloud is drawn in future !
	             i_sh = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, 1 + kijun, time, 0);
	         ...
	         IBar bar = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, 1, time, 0).get(0);

	         return bar.getClose() < cloudTop;
    }
	
	public boolean isIchiShortCancel(IHistory history, Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time) throws JFException
    {
	    double[][] 
	             // used for cloud borders, therefore moved kijun periods in past ! Cloud is drawn in future !
	             i_sh = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, 1 + kijun, time, 0);
	...
	public IchiDesc getIchi(IHistory history, Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
	    double[][] 
	             ...
	             i_sh = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, 1 + kijun, time, 0),
	             i_sh_1 = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, 2 + kijun, time, 0),
	             i_sh_2 = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, 3 + kijun, time, 0),
	             // lines are normally drawn...
	             lines = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, 3, time, 0);
	         ...
	         double
	             ...
	             i_cloudTop_1 = Math.round(Math.max(i_sh_1[SENOKU_A][0], i_sh_1[SENOKU_B][0]) * Math.pow(10, instrument.getPipScale())),
	             i_cloudBottom_1 = Math.round(Math.min(i_sh_1[SENOKU_A][0], i_sh_1[SENOKU_B][0]) * Math.pow(10, instrument.getPipScale())),
	             i_cloudTop_2 = Math.round(Math.max(i_sh_2[SENOKU_A][0], i_sh_1[SENOKU_B][0]) * Math.pow(10, instrument.getPipScale())),
	             i_cloudBottom_2 = Math.round(Math.min(i_sh_2[SENOKU_A][0], i_sh_1[SENOKU_B][0]) * Math.pow(10, instrument.getPipScale())),
	             i_kumo_A = Math.round(i_sh[CLOUD_A][0] * Math.pow(10, instrument.getPipScale())),
	             i_kumo_B = Math.round(i_sh[CLOUD_B][0] * Math.pow(10, instrument.getPipScale()));
	         ...
	         double
	             ...
	             cloudTop_1 = i_cloudTop_1 / Math.pow(10, instrument.getPipScale()),
	             cloudBottom_1 = i_cloudBottom_1 / Math.pow(10, instrument.getPipScale()),
	             kumo_A = i_kumo_A / Math.pow(10, instrument.getPipScale()),
	             kumo_B = i_kumo_B / Math.pow(10, instrument.getPipScale()),
	             ...
	             ATR = indicators.atr(instrument, pPeriod, side, 14, Filter.WEEKENDS, 1, time, 0)[0] * Math.pow(10, instrument.getPipScale());
	         ...
	         else if ((i_cloudTop > i_cloudTop_1 && i_cloudTop_1 >= i_cloudTop_2) || (i_cloudTop >= i_cloudTop_1 && i_cloudTop_1 > i_cloudTop_2))
	         ...
	         if (i_cloudBottom == i_cloudBottom_1 && i_cloudBottom_1 == i_cloudBottom_2)
	             res.bottomBorderDirection = new String("flat");
	         else if ((i_cloudBottom > i_cloudBottom_1 && i_cloudBottom_1 >= i_cloudBottom_2) || (i_cloudBottom >= i_cloudBottom_1 && i_cloudBottom_1 > i_cloudBottom_2))
    ...
    public double ichiCloudDistCalc(IHistory history, Instrument instrument, Period pPeriod, OfferSide side, IBar bar, long time, int lookBackPeriod) throws JFException {
        double[][] 
             // used for cloud borders, therefore moved kijun periods in past ! Cloud is drawn in future !
             i_sh = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, lookBackPeriod + kijun, time, 0);
         double[] atr_hist = indicators.atr(instrument, pPeriod, side, 14, Filter.WEEKENDS, lookBackPeriod, time, 0);
                     
         List<IBar> bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, lookBackPeriod, time, 0);
        ...
        double[][] 
             // used for cloud borders, therefore moved kijun periods in past ! Cloud is drawn in future !
             i_sh_now = indicators.ichimoku(instrument, pPeriod, side, tenkan, kijun, senkou, Filter.WEEKENDS, 1 + kijun, time, 0);
         double
             atr = indicators.atr(instrument, pPeriod, side, 14, Filter.WEEKENDS, 1, time, 0)[0],
             currDistance = 0.0,
             currDistanceAbs = 0.0,
