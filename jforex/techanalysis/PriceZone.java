...
public class PriceZone implements Comparable<PriceZone> {
	...
	public static PriceZone parsePriceZone(String input) {
		...
		String bottomStr, topStr;
		double bottomValue = -1.0, topValue = -1.0;
		...
		try {
			...
			System.err.print("PriceZone bottom " + bottomStr
					+ " can't be converted...");
		}
		try {
	...
	public String getHitText() {
		return new String(
				(direction.toUpperCase().equals("L") ? "Long entry zone "
						: "Short entry zone ")
						+ FXUtils.df4.format(bottom)
						+ " - " + FXUtils.df4.format(top) + " hit !");
	}

	/*
	 * PriceZones are sorted in descending order of bottom value, so they appear
	 * in lists as on the chart
	 */
	@Override
