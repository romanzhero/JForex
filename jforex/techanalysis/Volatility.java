...
public class Volatility {
	...
	public double[] getATRTimeSeries(Instrument instrument, Period pPeriod, OfferSide side, Filter filter, long time, int lookBack, int timeSeriesLength) throws JFException
	{		
		return indicators.atr(instrument, pPeriod, side, lookBack, filter, timeSeriesLength, time, 0);
	}
	...
	{		
    	double[][] bBands = indicators.bbands(instrument, pPeriod, side, AppliedPrice.CLOSE, lookBack, 2.0, 2.0, MaType.SMA, Filter.WEEKENDS, 1, time, 0);
    	double 
	...
	public double getBBandsSqueeze(Instrument instrument, Period pPeriod, OfferSide side, Filter filter, long time, int lookBack) throws JFException
	{		
    	double[][] bBands = indicators.bbands(instrument, pPeriod, side, AppliedPrice.CLOSE, lookBack, 2.0, 2.0, MaType.SMA, filter, 1, time, 0);
    	double 
    		twoStDev = bBands[0][0] - bBands[1][0],
    		twoATRs = 2 * getATR(instrument, pPeriod, side, time, lookBack); 
		return twoStDev / twoATRs * 100.0;
    ...
    public double getBBandsSqueezePercentile(Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, Filter filter, long time, int lookback, int historyBars) throws JFException {
	...
	private double[] getBBandsSqueezes(Instrument instrument, Period pPeriod, OfferSide side, AppliedPrice appliedPrice, Filter filter,	long time, int lookback, int historyBars) throws JFException {
    	...
    	for (int i = 0; i < ATRs.length; i++) {
	    	double 
	    		twoStDev = bBands[0][i] - bBands[1][i],
	    		twoATRs = 2 * ATRs[i]; 
