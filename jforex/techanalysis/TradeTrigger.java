...
public class TradeTrigger {
	...
	public class TriggerDesc {
		...
		public double 
			...
			combinedRealBodyPerc,
			combinedLowerHandlePerc;
		public boolean
			combinedRealBodyDirection; // true bullish, false bearish

		
		protected int numberOfBars = 0;
		protected IBar patternBars[] = null;

		public TriggerDesc(TriggerType type, 
							int noOfBars,
							IBar[] patternBars,
							double pivotLevel, 
							double srLevel, 
		...
		private double calcReversalSize() {
			...
			if (numberOfBars == 2) {
				double 
					patternHigh = Math.max(patternBars[0].getHigh(), patternBars[1].getHigh()),
					patternLow = Math.min(patternBars[0].getLow(), patternBars[1].getLow());
				return patternHigh - patternLow;
			}
			double 
				patternHigh = Math.max(patternBars[0].getHigh(), Math.max(patternBars[1].getHigh(), patternBars[2].getHigh())),
				patternLow = Math.min(patternBars[0].getLow(), Math.min(patternBars[1].getLow(), patternBars[2].getLow()));
			return patternHigh - patternLow;			
		}
		...
		public double getCombinedHigh() {
			...
			if (numberOfBars == 1)
				...
				return Math.max(patternBars[0].getHigh(), patternBars[1].getHigh());
			return Math.max(Math.max(patternBars[0].getHigh(), patternBars[1].getHigh()), patternBars[2].getHigh());
		}	
		...
		public double getCombinedLow() {
			...
			if (numberOfBars == 1)
				...
				return Math.min(patternBars[0].getLow(), patternBars[1].getLow());
			return Math.min(Math.min(patternBars[0].getLow(), patternBars[1].getLow()), patternBars[2].getLow());
		}	
	}
    ...
    /**
     * most generic bullish trigger, check for any of the 1-, 2- or 3-candles bullish reversal patterns
     * @return low of pivot bar if condition found (to serve as protective stop) otherwise -1
     */
    public double bullishReversalCandlePattern(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
		lastBullishTrigger = TriggerType.NONE;
    	double pivotLow = Double.MIN_VALUE;
    	if ((pivotLow = this.oneBarBullishTrigger(instrument, pPeriod, side, time)) != Double.MIN_VALUE) {
    		lastBullishTrigger = TriggerType.BULLISH_1_BAR;
    		return pivotLow;
    	}
    	else if ((pivotLow = this.twoBarsBullishTrigger(instrument, pPeriod, side, time)) != Double.MIN_VALUE) {
    		lastBullishTrigger = TriggerType.BULLISH_2_BARS;
    		return pivotLow;    		
    	}
    	if ((pivotLow = this.bullishPivotalLowsTrigger(instrument, pPeriod, side, time)) != Double.MIN_VALUE)
    	{
    ...
    public IBar bullishReversalCandlePatternBar(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
    	...
    	else if ((pivotLowBar = this.twoBarsBullishTriggerBar(instrument, pPeriod, side, time)) != null) {
    		lastBullishTrigger = TriggerType.BULLISH_2_BARS;
    		return pivotLowBar;    		
    	}
    ...
    {    	
    	...
    	else if ((pivotLowBar = this.bullishMaribouzuBar(instrument, pPeriod, side, time)) != null) {
    		lastBullishTrigger = TriggerType.BULLISH_1_BAR;
    		IBar[] patternBars = new IBar[1];
    		patternBars[0] = pivotLowBar;
    		// calc % of the combined candle
	...
	/**
     * support/resistance level is the same both for bullish and bearish pivot triangle - simply the middle bar middle
     */
	...
	public double priceKeltnerChannelPos(Instrument instrument, Period pPeriod, OfferSide side, long time, double price, int backBars) throws JFException {
    	double[] last2ATRs = indicators.atr(instrument, pPeriod, side, 14, Filter.WEEKENDS, 2, time, 0); 
    	double[] last2MAs = indicators.sma(instrument, pPeriod, side, AppliedPrice.CLOSE, 20, Filter.WEEKENDS, 2, time, 0);
    ...
    public double oneBarBullishTrigger(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
    ...
    public IBar bullishMaribouzuBar(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
    	List<IBar> bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, 1, time, 0);
        IBar currBar = bars.get(0);
        if (barLengthStatPos(instrument, pPeriod, side, currBar, 1000) <= 0.0)
    ...
    public double oneBarBearishTrigger(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
        ...
        double 
        	barLength = currBar.getHigh() - currBar.getLow(),
        	realBodyHigh = Math.max(currBar.getOpen(), currBar.getClose()),
        	upperWick = currBar.getHigh() - realBodyHigh;
    ...
    public double bearishMaribouzuBar(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
        ...
        if (currBar.getClose() < currBar.getOpen() && (currBar.getOpen() - currBar.getClose()) / barLength >= 0.8)
        {
        	return currBar.getHigh();
        }
        else
    ...
    public double twoBarsBullishTrigger(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
    	List<IBar> bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, 2, time, 0);
        IBar prevBar = bars.get(0);
        IBar currBar = bars.get(1);
        
        // bullish inside bar (previous very big and RED, current bar very small)
        if (barBodyLengthStatPos(instrument, pPeriod, side, currBar, 1000) <= -1.0
        	&& barBodyLengthStatPos(instrument, pPeriod, side, prevBar, 1000) >= 1.0	
    	...
    	int[] engulfing = indicators.cdlEngulfing(instrument, pPeriod, side, Filter.WEEKENDS, 1, time, 0);
    	if (engulfing != null && engulfing[0] == 100)
    		return prevBar.getLow() < currBar.getLow() ? prevBar.getLow() : currBar.getLow();
        ...
        if (bodiesOverlapPerc(prevBar, currBar) >= 0.8)
        	// careful: pivot bar is the one with deeper low, not necessarily previous !
    ...
    public String bullishCandleDescription(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
    	// first check strict engulfing possibility
    	int[] engulfing = indicators.cdlEngulfing(instrument, pPeriod, side, Filter.WEEKENDS, 1, time, 0);
    	if (engulfing != null && engulfing[0] == 100)
    	...
    	int[] abandonedBaby = indicators.cdlAbandonedBaby(instrument, pPeriod, side, 60.0, Filter.WEEKENDS, 1, time, 0);
    ...
    public double barsUpperHandlePerc(IBar bar) {
    	double totalBarSize = bar.getHigh() - bar.getLow();
    	double upperHandleSize = bar.getHigh() - Math.max(bar.getClose(), bar.getOpen());  	
    ...
    public double barsLowerHandlePerc(IBar bar) {
    	double totalBarSize = bar.getHigh() - bar.getLow();
    	double lowerHandleSize = Math.min(bar.getClose(), bar.getOpen()) - bar.getLow();  	
    ...
    public double twoBarsBearishTrigger(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
        ...
        if (bodiesOverlapPerc(prevBar, currBar) >= 0.8) {
        	// careful: pivot bar is the one with higher high, not necessarily the previous one !
	...
	private double bodiesOverlapPerc(IBar prevBar, IBar currBar) {
        ...
        double bar1RealBodyLow = Math.min(prevBar.getOpen(), prevBar.getClose());
        double bar1RealBodyHigh = Math.max(prevBar.getOpen(), prevBar.getClose());
        double bar1RealBodyLength = bar1RealBodyHigh - bar1RealBodyLow;

        double bar2RealBodyLow = Math.min(currBar.getOpen(), currBar.getClose());
        double bar2RealBodyHigh = Math.max(currBar.getOpen(), currBar.getClose());
      
    	double bodiesOverlapPerc = 0; 

        if (bar2RealBodyHigh >= bar1RealBodyHigh)
        	bodiesOverlapPerc = (bar1RealBodyHigh - bar2RealBodyLow) / bar1RealBodyLength; 
        else
        	bodiesOverlapPerc = (bar2RealBodyHigh - bar1RealBodyLow) / bar1RealBodyLength;
		return bodiesOverlapPerc;
	}


    /**
     * Two criteria to cancel a pending buy entry order:
     * 1. low of the last bar was below "bottom" of the initial signal candle(s)
     * 2. bearish pivotal highs but only if also two lower lows in its last two candles 
     ...
     * @throws JFException
     */
    public boolean longEntryCancel(double initialStopLoss, Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
    	List<IBar> bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, 3, time, 0);
        IBar currBar = bars.get(2);
    	
        if (currBar.getLow() < initialStopLoss) return true;
        IBar prevBar1 = bars.get(1);
        IBar prevBar2 = bars.get(0);
      
        return  currBar.getHigh() < prevBar1.getHigh() && prevBar1.getHigh() > prevBar2.getHigh()
	...
	public double triggerFoundBearishPivotalHighs(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
        ...
        if (currBar.getHigh() < prevBar1.getHigh() && prevBar1.getHigh() > prevBar2.getHigh())
        {
	...
	public boolean startT1BL(IOrder positionOrder, Instrument instrument, Period pPeriod, OfferSide side, IIndicators.AppliedPrice appliedPrice, long time) throws JFException
    {
		// Logic if following:
		// As long as high of the bar is ABOVE top of BBands, don't start trailing
		// If both Stochs go (even ONCE !) above 90 or ADX go above 40, start trailing on FIRST bar which high is NOT above BBands anymore
		// BUT: independent of these two, as soon as there's a bar with LOW above BBands top start trailing !
        ...
        if (!stochsHeavilyOverboughtTriggered)
        {
            ...
            this.stochsHeavilyOverboughtTriggered = fastStoch >= 89.0 && slowStoch >= 89.0;
            if (this.stochsHeavilyOverboughtTriggered)
            {
                ...
                log.printAction(Logger.logTags.ORDER.toString(), 
            			...
            			Logger.logTags.TRAILING_CONDITION_FOUND.toString() + ": StochFast = " + df1.format(fastStoch) + ", StochSlow = " + df1.format(slowStoch), 
            			positionOrder.getClosePrice(), 
            			(positionOrder.getStopLossPrice() - positionOrder.getOpenPrice()) * 10000, 
        ...
        {        	
        	if (prevBar.getHigh() < bBandsTop)
        	{
                ...
                log.printAction(Logger.logTags.ORDER.toString(), 
            			positionOrder.getLabel(), 
            			FXUtils.getFormatedTimeGMT(time), 
            			Logger.logTags.TRAILING_CONDITION_FOUND.toString() + ": bar's high below BBands top (" + 
            				df1.format(prevBar.getHigh()) + " / " + df1.format(bBandsTop) + ")", 
            			positionOrder.getClosePrice(), 
            			(positionOrder.getStopLossPrice() - positionOrder.getOpenPrice()) * 10000, 
            			positionOrder.getProfitLossInPips(), 0);        		
        	}
	...
	public double bearishReversalCandlePattern(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException {
    	...
    	else if ((pivotHigh = this.twoBarsBearishTrigger(instrument, pPeriod, side, time)) != Double.MAX_VALUE) {
    		lastBearishTrigger = TriggerType.BEARISH_2_BARS;
    		return pivotHigh;
    		
    	}
        else if ((pivotHigh = this.triggerFoundBearishPivotalHighs(instrument, pPeriod, side, time)) != Double.MAX_VALUE) {
	...
	public TriggerDesc bearishReversalCandlePatternDesc(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException {
    	...
    	if ((pivotHigh = this.oneBarBearishTrigger(instrument, pPeriod, side, time)) != Double.MAX_VALUE) {
    		...
    		return new TriggerDesc(TriggerType.BEARISH_1_BAR,
    				...
    				(bodyTop - bodyBottom) / range * 100.0,
    				(bodyBottom - patternBars[0].getLow()) / range * 100.0,
    		...
    		double 
    			h = pivotHigh,
    			l = Math.min(Math.min(firstBar.getLow(), middleBar.getLow()), currBar.getLow()),
    			o = firstBar.getOpen(),
    			c = currBar.getClose(),
    			range = h - l,
    			bodyTop = Math.max(o, c),
        ...
        else if ((pivotHigh = this.bearishMaribouzuBar(instrument, pPeriod, side, time)) != Double.MAX_VALUE) {
    		lastBearishTrigger = TriggerType.BEARISH_1_BAR;
        	List<IBar> bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, 1, time, 0);
    		IBar[] patternBars = new IBar[1];
    		patternBars[0] = bars.get(0);
	 ...
	 * @return in pips
	 */
	public double avgBarLength(Instrument instrument, Period pPeriod, OfferSide side, IBar forBar, int lookBack) throws JFException {
    	List<IBar> bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, lookBack - 1, forBar.getTime(), 0);
    	// include current bar in calc !
    	bars.add(forBar);
    	if (bars.size() == 0)
		...
		return (barLengths / bars.size()) * Math.pow(10, instrument.getPipScale());
	}
    ...
    public double barBodyLengthStatPos(Instrument instrument, Period pPeriod, OfferSide side, IBar forBar, int lookBack) throws JFException {
    	...
    	int i = 0;
    	for (IBar currBar : bars) {
    		barBodyLengths[i++] = Math.max(currBar.getOpen(), currBar.getClose()) - Math.min(currBar.getOpen(), currBar.getClose());
    	}
    	double[] stDevStats = FXUtils.sdFast(barBodyLengths); 
		double forBbarBodyLength = Math.max(forBar.getOpen(), forBar.getClose()) - Math.min(forBar.getOpen(), forBar.getClose());
		return (forBbarBodyLength - stDevStats[0]) / stDevStats[1];
	}

    public double avgHandleLength(Instrument instrument, Period pPeriod, OfferSide side, IBar forBar, int lookBack) throws JFException {
    	List<IBar> bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, lookBack - 1, forBar.getTime(), 0);
    	// include current bar in calc !
    	bars.add(forBar);
    	...
    	for (IBar currBar : bars) {
    		double 
    			bodyTop = Math.max(currBar.getOpen(), currBar.getClose()),
    			bodyBottom = Math.min(currBar.getOpen(), currBar.getClose());
    ...
    /**
     * Two criteria to cancel a pending sell entry order:
     * 1. high of the last bar was above "top" of the initial signal candle(s)
     * 2. bullish pivotal lows but only if also two higher highs in its last two candles 
     * @param initialStopLoss
     * @param instrument
     * @param pPeriod
     * @param side
     ...
     * @throws JFException
     */
	public boolean shortEntryCancel(double initialStopLoss, Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException {
    	List<IBar> bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, 3, time, 0);
        IBar currBar = bars.get(2);
    	
        if (currBar.getHigh() > initialStopLoss) return true;
        
        IBar prevBar1 = bars.get(1);
        IBar prevBar2 = bars.get(0);
      
        return  currBar.getLow() > prevBar1.getLow() && prevBar1.getLow() < prevBar2.getLow()
        		&& currBar.getHigh() > prevBar1.getHigh();
	}

	public boolean startT1BH(IOrder positionOrder, Instrument instrument, Period pPeriod, OfferSide side, AppliedPrice close, long time) throws JFException {
		// Logic if following:
		// As long as low of the bar is BELOW bottom of BBands, don't start trailing
		// If both Stochs go (even ONCE !) below 10 or ADX go above 40, start trailing on FIRST bar which low is NOT below BBands anymore
		// BUT: independent of these two, as soon as there's a bar with HIGH below BBands top start trailing !
    	double[][] bBands = indicators.bbands(instrument, pPeriod, side, AppliedPrice.CLOSE, 20, 2.0, 2.0, MaType.SMA, 
    			Filter.WEEKENDS, 1, time, 0);
        ...
        if (!stochsHeavilyOversoldTriggered)
        {
            ...
            this.stochsHeavilyOversoldTriggered = fastStoch <= 11.0 && slowStoch <= 11.0;
            if (this.stochsHeavilyOversoldTriggered)
            {
                ...
                log.printAction(Logger.logTags.ORDER.toString(), 
            			...
            			Logger.logTags.TRAILING_CONDITION_FOUND.toString() + ": StochFast = " + df1.format(fastStoch) + ", StochSlow = " + df1.format(slowStoch), 
            			positionOrder.getClosePrice(), 
            			(positionOrder.getOpenPrice() - positionOrder.getStopLossPrice()) * 10000, 
            			positionOrder.getProfitLossInPips(), 0);
            }
        }
        ...
        {        	
        	if (prevBar.getLow() > bBandsBottom)
        	{
                ...
                log.printAction(Logger.logTags.ORDER.toString(), 
            			positionOrder.getLabel(), 
            			FXUtils.getFormatedTimeGMT(time), 
            			Logger.logTags.TRAILING_CONDITION_FOUND.toString() + ": bar's low above BBands bottom (" + 
            				df1.format(prevBar.getLow()) + " / " + df1.format(bBandsBottom) + ")", 
            			positionOrder.getClosePrice(), 
            			(positionOrder.getStopLossPrice() - positionOrder.getOpenPrice()) * 10000, 
            			positionOrder.getProfitLossInPips(), 0);        		
        	}
    ...
    public double previousBarOverlap(Instrument instrument, Period pPeriod, OfferSide side, long time) throws JFException
    {
    	List<IBar> bars = history.getBars(instrument, pPeriod, side, Filter.WEEKENDS, 2, time, 0);
        IBar prevBar = bars.get(0);
        IBar currBar = bars.get(1);
        ...
        if (currBar.getHigh() > prevBar.getHigh()) {
        	if (currBar.getLow() >= prevBar.getHigh())
        		...
        		return (prevBar.getHigh() - currBar.getLow()) / prevBarHeight * 100.0;
        	else 
        		return 100.0;
        	...
        	return (currBar.getHigh() - prevBar.getLow()) / prevBarHeight * 100.0;
        else
        	return (currBar.getHigh() - currBar.getLow()) / prevBarHeight * 100.0;
