...
public class JForexIchiStrategy extends BasicStrategy implements IStrategy {
	...
	public void onStart(IContext context) throws JFException {
		...
		strategy = new IchiForexStrategy(3, new JForexBrokerEngine(engine,
				context.getAccount()), TimeFrames.FOUR_HOURS);
	...
	public void onBar(Instrument instrument, Period period, IBar askBar,
		...
		if (skipPairs.contains(instrument))
			return;
		String targetTimeFrame = pairsTimeFrames.get(instrument), currentTimeFrame = FXUtils.timeFrameNamesMap
				.get(period.toString());
		if (currentTimeFrame == null
				|| !currentTimeFrame.equals(targetTimeFrame))
			return;

		strategy.processTAEvent(instrument.toString(),
				TimeFramesMap.fromJForexMap.get(period), bidBar.getTime(),
				taEventsSource);
	}
	...
	public void onMessage(IMessage message) throws JFException {
		...
		if (message.getType().equals(IMessage.Type.ORDER_FILL_OK)) {
			IBrokerEvent event = new OrderFillEvent(message.getOrder()
					.getInstrument().toString(), message.getOrder().getLabel(),
					message.getCreationTime(), message.getOrder().getAmount(),
					message.getOrder().getOpenPrice());
			strategy.processBrokerEvent(message.getOrder().getInstrument()
					.toString(), event, taEventsSource);
			return;
		}

		if (message.getType().equals(IMessage.Type.ORDER_CLOSE_OK)) {
			IBrokerEvent event = new OrderSLCloseEvent(message.getOrder()
					.getInstrument().toString(), message.getOrder().getLabel(),
