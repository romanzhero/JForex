...
public class Adaptive_MR_TF_Strategy extends AbstractMultiPositionStrategy
	...
	long lastFirstExitSignal = 0; // its time. Important not to check further
									// signals on day of its appearance
	...
	public void onBar(Instrument instrument, Period period, IBar askBar,
		...
		case ENTRY_ORDER_WAITING:
			...
			break;
		case POSITION_OPENED: // includes all situations regardless of # of open
								// orders before 1st exit signal
			if (exitSignalFirstFound(instrument, period, askBar, bidBar)) {
				lastFirstExitSignal = bidBar.getTime();
				tradeState.stateTransition(
						TradeState.PHASE_1_EXIT_SIGNAL_FOUND, bidBar.getTime());
		...
		case POSITION_OPENED_AND_ENTRY_ORDER_WAITING:
			checkForCancelEntryOrder(instrument, period, bidBar);
			break;
		case POSITION_OPENED_MAX_REACHED: // can be reached only if 1st exit
											// signal was never given !
			if (exitSignalFirstFound(instrument, period, askBar, bidBar)) {
				lastFirstExitSignal = bidBar.getTime();
				tradeState.stateTransition(
						TradeState.PHASE_2_1ST_EXIT_SIGNAL_FOUND,
						bidBar.getTime());
				// move all SL levels to break-even !
				toBreakEvenAll();
			}
			break;
		case PHASE_1_EXIT_SIGNAL_FOUND: // 1st exit signal occurred with only
										// one position
			checkOppositeSignalOnePosition(instrument, period, askBar, bidBar);
			// alternative to test: SL level of #1 position moved to this
			// trigger level
			break;
		...
		case EXIT_ORDER_TRAILING: // situation where 2nd exit signal occurred !
			// stop loss / exit levels are set only here, on their own 4h
			// signals
			checkOppositeSignal(instrument, period, askBar, bidBar);
			break;
		default:
	...
	private void checkOppositeSignalOnePosition(Instrument instrument,
		...
		if (strategyState == StrategyStates.LONG_POSITION) {
			if (eventsSource.any4hBearishTrigger(instrument, period, bidBar,
					logLine)) {
				setStopLossPrice(bidBar.getLow());
			}
		} else {
			if (eventsSource.any4hBullishTrigger(instrument, period, bidBar,
					logLine)) {
				setStopLossPrice(bidBar.getHigh());
			}
		}
	...
	private void checkOppositeSignal(Instrument instrument, Period period,
	...
	private void checkForNextSignal(Instrument instrument, Period period,
		...
		if (!FXUtils.tradingDistanceOK(bidBar.getTime(), timeOfLastTrade
				+ INTERVAL_BETWEEN_TRADES))
			return;
		...
		if (strategyState == StrategyStates.LONG_POSITION) {
			if (eventsSource.strong4hBullishNextEntrySignal(instrument, period,
					bidBar, logLine)) {
				double stopLossValue = mailValuesMap.get("bullishPivotLevel4h")
						.getDoubleValue();
				...
				log.printAction(
						...
						(protectiveStop - waitingOrder.getOpenPrice())
								* Math.pow(10, instrument.getPipScale()), 0, 0);
			}
		} else if (eventsSource.strong4hBearishNextEntrySignal(instrument,
				period, bidBar, logLine)) {
			double stopLossValue = mailValuesMap.get("bearishPivotLevel4h")
					.getDoubleValue();
			...
			log.printAction(
					...
					waitingOrder.getOpenPrice(),
					(waitingOrder.getOpenPrice() - protectiveStop)
							* Math.pow(10, instrument.getPipScale()), 0, 0);

		}
	}

	private boolean exitSignalFirstFound(Instrument instrument, Period period,
			IBar askBar, IBar bidBar) throws JFException {
		// next checks can be done only once NEW daily signal is formed, + at
		// least 1 full day AFTER last entry
		...
		eventsSource.collectAllStats(instrument, period, bidBar, logLine);
		if (strategyState == StrategyStates.LONG_POSITION) {
			return eventsSource.bullishExitSignalFirst(instrument, period,
					bidBar, logLine);
			// return eventsSource.strong4hBearishFirstEntrySignal(instrument,
			// period, bidBar, logLine);
		} else {
			return eventsSource.bearishExitSignalFirst(instrument, period,
					bidBar, logLine);
			// return eventsSource.strong4hBullishFirstEntrySignal(instrument,
			// period, bidBar, logLine);
		}
	}

	private boolean exitSignalSecondFound(Instrument instrument, Period period,
			IBar askBar, IBar bidBar) throws JFException {
		...
		eventsSource.collectAllStats(instrument, period, bidBar, logLine);
		if (strategyState == StrategyStates.LONG_POSITION) {
			if (eventsSource.bullishExitSignalSecond(instrument, period,
					bidBar, logLine))
				return true;
			return eventsSource.bullishExitSignalFirst(instrument, period,
					bidBar, logLine);
			...
			if (eventsSource.bearishExitSignalSecond(instrument, period,
					bidBar, logLine))
				return true;
			return eventsSource.bearishExitSignalFirst(instrument, period,
					bidBar, logLine);
	...
	protected boolean noExitSignalDueToTimeChecks(IBar bidBar) {
		DateTime currentBarTime = new DateTime(bidBar.getTime()), currentDay = new DateTime(
				currentBarTime.getYear(), currentBarTime.getMonthOfYear(),
				currentBarTime.getDayOfMonth(), 0, 0, 0, 0), lastSignalTime = new DateTime(
				lastFirstExitSignal), lastSignalDay = new DateTime(
				lastSignalTime.getYear(), lastSignalTime.getMonthOfYear(),
				lastSignalTime.getDayOfMonth(), 0, 0, 0, 0);
		if (currentDay.equals(lastSignalDay))
			return true;

		// also doesn't make sense to check for 1d bar in which LAST (??) ENTRY
		// signal appeared, as well as the next !
		DateTime firstEntrySignalTime = new DateTime(orders.get(
				orders.size() - 1).getCreationTime(), DateTimeZone.forID("UTC")), firstEntrySignalDay = new DateTime(
				firstEntrySignalTime.getYear(),
				firstEntrySignalTime.getMonthOfYear(),
				firstEntrySignalTime.getDayOfMonth(), 0, 0, 0, 0,
				DateTimeZone.forID("UTC")), lastDay = FXUtils
				.getLastTradingDay(bidBar.getTime());
	...
	protected boolean noCancelTimeCheck(IBar bidBar) {
		...
		// also doesn't make sense to check for 1d bar in which LAST (??) ENTRY
		// signal appeared, as well as the next !
		DateTime waitingEntrySignalTime = new DateTime(
				waitingOrder.getCreationTime(), DateTimeZone.forID("UTC")), waitingEntrySignalDay = new DateTime(
				waitingEntrySignalTime.getYear(),
				waitingEntrySignalTime.getMonthOfYear(),
				waitingEntrySignalTime.getDayOfMonth(), 0, 0, 0, 0,
				DateTimeZone.forID("UTC")), lastDay = FXUtils
				.getLastTradingDay(bidBar.getTime());
	...
	private void checkForCancelEntryOrder(Instrument instrument, Period period,
			IBar bidBar) throws JFException {
		...
		if (waitingOrder.isLong()) {
			if (eventsSource.bullishEntryNoGoZone(instrument, period, bidBar,
					logLine)
					|| eventsSource.strong4hBearishFirstEntrySignal(instrument,
							period, bidBar, logLine)) {
				waitingOrder.close();
				...
				log.printAction(
						...
						(waitingOrder.getOpenPrice() - protectiveStop)
								* Math.pow(10, instrument.getPipScale()), 0, 0);
			} else if (eventsSource.strong4hBullishNextEntrySignal(instrument,
					period, bidBar, logLine)) {
			...
			if (eventsSource.bearishEntryNoGoZone(instrument, period, bidBar,
					logLine)
					|| eventsSource.strong4hBullishFirstEntrySignal(instrument,
							period, bidBar, logLine)) {
				waitingOrder.close();
				// Careful, waitingOrder.close will trigger onMessage call !
				log.printAction(
	...
	private void checkForNewSignal(Instrument instrument, Period period,
			IBar askBar, IBar bidBar) throws JFException {
		...
		if (eventsSource.strong4hBullishFirstEntrySignal(instrument, period,
				bidBar, logLine)) {
			double stopLossValue = mailValuesMap.get("bullishPivotLevel4h")
					.getDoubleValue();
			placeBuyOrder(instrument, period, askBar, bidBar, stopLossValue);
			...
			log.printAction(
					...
					(protectiveStop - waitingOrder.getOpenPrice())
							* Math.pow(10, instrument.getPipScale()), 0, 0);
		} else if (eventsSource.strong4hBearishFirstEntrySignal(instrument,
				period, bidBar, logLine)) {
			double stopLossValue = mailValuesMap.get("bearishPivotLevel4h")
					.getDoubleValue();
			placeSellOrder(instrument, period, askBar, bidBar, stopLossValue);
	...
	public void onMessage(IMessage message) throws JFException {
		...
		case SCANNING_TA_SIGNALS:
			if (message.getType().equals(IMessage.Type.ORDER_SUBMIT_OK)) {
				if (message.getOrder().isLong())
					waitingOrder.setStopLossPrice(protectiveStop
							- safeZone
		...
		case POSITION_OPENED:
			if (message.getType().equals(IMessage.Type.ORDER_SUBMIT_OK)) {
				if (message.getOrder().isLong())
					waitingOrder.setStopLossPrice(protectiveStop - safeZone
							/ 10000);
				else
					waitingOrder.setStopLossPrice(protectiveStop + safeZone
							/ 10000);
		...
		case POSITION_OPENED_MAX_REACHED:
			// this happens only when SL is hit for some (or all ?!) of the open
			// orders...
			if (message.getType().equals(IMessage.Type.ORDER_CLOSE_OK)) {
				...
				if (orders.size() == 0)
					...
					// if not all the orders are closed that means there are
					// free slots now !
					tradeState.stateTransition(TradeState.POSITION_N_OPENED,
							message.getCreationTime());
			} else if (message.getType().equals(IMessage.Type.ORDER_CHANGED_OK)) {
				String msgSuffix = new String(" (" + lastStopType.toString()
						+ ")");
		...
		case POSITION_OPENED_AND_ENTRY_ORDER_WAITING:
			...
			// message will be send for all the open orders !
				...
				if (!message.getOrder().equals(waitingOrder)) {
					...
					if (!waitingOrder.getState().equals(IOrder.State.CREATED)
							&& !waitingOrder.getState().equals(
									IOrder.State.CANCELED))
						waitingOrder.close();
				} else
					// order canceled
			...
			break;
		case PHASE_1_EXIT_SIGNAL_FOUND: // only one position and exit signal
										// appeared, SL level is getting changed
			if (message.getType().equals(IMessage.Type.ORDER_CLOSE_OK)) {
				processPositionsCloseBare(message, false);
				if (orders.size() == 0)
					strategyState = StrategyStates.NO_POSITION;
			...
			if (message.getType().equals(IMessage.Type.ORDER_SUBMIT_OK)) {
				if (message.getOrder().isLong())
					...
					waitingOrder.setStopLossPrice(protectiveStop + safeZone
							/ 10000);
				tradeState.stateTransition(
						TradeState.POSITION_N_AND_ENTRY_ORDER_WAITING,
						message.getCreationTime());
			}
			// message will be send for all the open orders !
			else if (message.getType().equals(IMessage.Type.ORDER_CLOSE_OK)) {
		...
		case PHASE_2_1ST_EXIT_SIGNAL_FOUND:
			if (message.getType().equals(IMessage.Type.ORDER_CHANGED_OK)) {
				...
				if (orders.size() == 0)
					...
					tradeState.stateTransition(TradeState.PHASE_2_POSITION_N,
							message.getCreationTime());
			}
			break;
		case PHASE_2_POSITION_N:
