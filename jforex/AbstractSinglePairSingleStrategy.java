...
public abstract class AbstractSinglePairSingleStrategy extends
	...
	public AbstractSinglePairSingleStrategy(Properties p) {
		super(p);
		// expect that correct validation and correct object type setting have
		// been done already

		trendStDevDefinitionMin = Double.parseDouble(p
				.getProperty("trendStDevDefinitionMin"));
		trendStDevDefinitionMax = Double.parseDouble(p
				.getProperty("trendStDevDefinitionMax"));

		useFilters = Boolean.parseBoolean(p.getProperty("USE_FILTERS", "false")
				.equalsIgnoreCase("yes") ? "true" : "false");
		reportFilters = Boolean.parseBoolean(p.getProperty("REPORT_FILTERS",
				"false").equalsIgnoreCase("yes") ? "true" : "false");

		// now get all the filter settings. Filtering is done only for members
		// of this set
		if (p.containsKey(EntryFilters.FILTER_MOMENTUM.toString())
				&& p.getProperty(EntryFilters.FILTER_MOMENTUM.toString())
						.equals("yes"))
			entryFilters.add(EntryFilters.FILTER_MOMENTUM);
		if (p.containsKey(EntryFilters.FILTER_ENTRY_BAR_HIGH.toString())
				&& p.getProperty(EntryFilters.FILTER_ENTRY_BAR_HIGH.toString())
						.equals("yes"))
			entryFilters.add(EntryFilters.FILTER_ENTRY_BAR_HIGH);
		if (p.containsKey(EntryFilters.FILTER_ENTRY_BAR_LOW.toString())
				&& p.getProperty(EntryFilters.FILTER_ENTRY_BAR_LOW.toString())
						.equals("yes"))
			entryFilters.add(EntryFilters.FILTER_ENTRY_BAR_LOW);
		if (p.containsKey(EntryFilters.FILTER_4H.toString())
				&& p.getProperty(EntryFilters.FILTER_4H.toString()).equals(
						"yes"))
	...
	public void onStartExec(IContext context) throws JFException {
		...
		for (Object key : keys) {
			...
			if (strKey.startsWith(FilterFactory.FILTER_PREFIX)) {
				filtersConfiguration.setProperty(strKey,
						conf.getProperty(strKey));
				allFilters.put(
						strKey,
						FilterFactory.createFilter(strKey,
								conf.getProperty(strKey), indicators, history));
			}
		}
		...
		for (IOrder entryOrder : engine.getOrders()) {
			if (entryOrder.getState().equals(IOrder.State.OPENED)) {
				log.print("Open order " + entryOrder.getLabel()
						+ " found; Setting state to "
						+ TradeState.ENTRY_ORDER_WAITING);
				tradeState.stateTransition(TradeState.ENTRY_ORDER_WAITING,
						new DateTime().getMillis());
				break;
			}
			if (entryOrder.getState().equals(IOrder.State.FILLED)) {
				log.print("Position with order " + entryOrder.getLabel()
						+ " found; Current profit/loss: "
	...
	protected void placeBuyOrder(Instrument instrument, Period period,
		...
		positionOrder = engine.submitOrder(getLabel(instrument), instrument,
