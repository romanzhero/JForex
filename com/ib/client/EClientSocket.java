...
public class EClientSocket {
	...
	protected void connectionError() {
		m_anyWrapper.error(EClientErrors.NO_VALID_ID,
	...
	public synchronized void reqContractDetails(int reqId, Contract contract) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_REQCONTRACT, "" + e);
			close();
		}
	}
	...
	public synchronized void placeOrder(int id, Contract contract, Order order) {
		...
		try {
			...
			if (m_serverVersion >= MIN_SERVER_VER_SMART_COMBO_ROUTING_PARAMS
				...
				int smartComboRoutingParamsCount = smartComboRoutingParams == null ? 0
						: smartComboRoutingParams.size();
				send(smartComboRoutingParamsCount);
				if (smartComboRoutingParamsCount > 0) {
					for (int i = 0; i < smartComboRoutingParamsCount; ++i) {
						TagValue tagValue = (TagValue) smartComboRoutingParams
								.get(i);
						send(tagValue.m_tag);
	...
	public synchronized void reqAccountUpdates(boolean subscribe,
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_ACCT, ""
					+ e);
			close();
		}
	}
	...
	public synchronized void reqExecutions(int reqId, ExecutionFilter filter) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_EXEC, ""
					+ e);
			close();
		}
	}
	...
	public synchronized void reqOpenOrders() {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_OORDER, ""
					+ e);
			close();
		}
	}
	...
	public synchronized void reqIds(int numIds) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_CORDER, ""
					+ e);
			close();
		}
	}
	...
	public synchronized void setServerLogLevel(int logLevel) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_SERVER_LOG_LEVEL, "" + e);
			close();
		}
	}
	...
	public synchronized void reqCurrentTime() {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_REQCURRTIME, "" + e);
			close();
		}
	}
	...
	public synchronized void calculateImpliedVolatility(int reqId,
			Contract contract, double optionPrice, double underPrice) {
	...
	public synchronized void reqGlobalCancel() {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_REQGLOBALCANCEL, "" + e);
			close();
		}
	}
	...
	public synchronized void reqMarketDataType(int marketDataType) {
		...
		try {
			...
			error(EClientErrors.NO_VALID_ID,
					EClientErrors.FAIL_SEND_REQMARKETDATATYPE, "" + e);
			close();
		}
	}
